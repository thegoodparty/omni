...
// `HUBSPOT_TRANSACTIONAL_TOKEN` private-app secret and the `MAGIC_LINK_EMAIL_ID`
// transactional email. The send is best-effort: if it (or its config) fails, we
// still return the `url` so the rep can copy it manually, alongside a `sent`
// boolean and an optional `sendError`.
//
...
//   - 'generate' (default): provision the user, mint a new link, email it. When
//     `phone` is also supplied, gp-api texts the link in the same call and the
//     response carries `smsSent` / `smsError` alongside `sent` / `sendError`.
...
async function sendMagicLinkEmail({ email, firstName, lastName, url, userId }) {
  ...
  const emailId = process.env.MAGIC_LINK_EMAIL_ID
  if (!transactionalToken || !emailId) {
    return {
      sent: false,
      sendError:
        'Email auto-send is not configured (HUBSPOT_TRANSACTIONAL_TOKEN and MAGIC_LINK_EMAIL_ID secrets are required).',
    }
  }
  ...
  try {
    const res = await fetch(SINGLE_SEND_URL, {
      ...
      body: JSON.stringify({
        ...
        message: { to: email, sendId: `eo-magic-${userId}-${Date.now()}` },
...
async function fetchExistingMagicLink({ gpApiUrl, token, email }) {
  try {
    const res = await fetch(
      `${gpApiUrl}/v1/admin/elected-office/magic-link?email=${encodeURIComponent(email)}`,
      {
        method: 'GET',
...
async function textExistingMagicLink({
  ...
  try {
    const res = await fetch(
      `${gpApiUrl}/v1/admin/elected-office/magic-link/sms`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
exports.main = async (context = {}) => {
  ...
  try {
    const res = await fetch(`${gpApiUrl}/v1/admin/elected-office/magic-link`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
