...
// for confirmation before generating the link. The `win_magic_link_*` /
// `win_onboarding_completed_at` properties are written by gp-api (it mirrors the
...
const CONTACT_PROPERTIES = [
  ...
  'phone',
  'mobilephone',
  'win_magic_link_status',
  'win_magic_link_sent_at',
  'win_magic_link_expires_at',
  'win_magic_link_redeemed_at',
  'win_onboarding_completed_at',
]
...
hubspot.extend<'crm.record.tab'>(({ actions, context }) => (
  <WinMagicLinkCard actions={actions} context={context} />
))
...
// `win_magic_link_status`) so expiry flips correctly at read time. Progress
...
function deriveLifecycle(
  ...
  const completedAt = parseHsDate(properties?.win_onboarding_completed_at)
  if (completedAt) return { state: 'completed', completedAt }

  const redeemedAt = parseHsDate(properties?.win_magic_link_redeemed_at)
  if (redeemedAt) return { state: 'redeemed', redeemedAt }

  const sentAt = parseHsDate(properties?.win_magic_link_sent_at)
  if (!sentAt) return { state: 'none' }

  const expiresAt = parseHsDate(properties?.win_magic_link_expires_at)
...
function WinMagicLinkCard({
  ...
  // candidate hasn't finished onboarding (e.g. expired, or a stalled redeemer).
  const canRegenerate =
    ...
    <Flex direction="column" gap="md">
      ...
      {lifecycle.state === 'completed' ? (
        <Alert title="Onboarding complete" variant="success">
          This candidate launched their campaign
      ...
      {lifecycle.state === 'redeemed' && !freshUrl ? (
        <Alert title="Link redeemed — onboarding in progress" variant="info">
          This candidate signed in with their magic link
          {lifecycle.redeemedAt
            ? ` on ${formatDate(lifecycle.redeemedAt)}`
            : ''}{' '}
          and is setting up their campaign.
        </Alert>
      ) : null}
      ...
      <Flex direction="column" gap="xs">
        ...
        {phone ? (
          <>
            ...
            >
              This candidate agreed on this call to be texted their sign-in link.
            </Checkbox>
          </>
        ) : (
          <Text variant="microcopy">
