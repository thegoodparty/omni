...
// political party (mapPoliticalParty in transformToPersonOutput.utils.ts) with
...
// Party a non-null, non-empty value falls back to when it matches no rule, and
// the value a null/blank `Parties_Description` maps to. Both are 'Other' in the
// current display logic (`if (!value) return 'Other'` + trailing `return
// 'Other'`).
export const POLITICAL_PARTY_FALLBACK = 'Other' satisfies CanonicalParty

export type PoliticalPartyRule = {
  // Canonical party this rule resolves to. 'Other' is intentionally excluded:
  // it is the fallback, never a positively-matched rule.
...
// Classify a raw `Parties_Description` value exactly as the display path does.
// This is the shared implementation mapPoliticalParty delegates to, so display
// output is guaranteed identical to the historical inline logic.
