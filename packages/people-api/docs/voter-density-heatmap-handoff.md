...
> **Status:** app side (people-api / gp-api / gp-marketing) is built against the
> contract in this doc and is live-behind-empty-data. This document is the spec
> for the **data** half. Nothing here has been implemented in
> `gp-data-platform` — it is the deliverable for that team.
...
into the people-api Postgres. people-api serves it read-only over S2S; gp-api
proxies it to the public page.
...
### 3.1 `int__people_api__voter_h3.sql` (intermediate, ephemeral or table)
...
### 3.2 `m_people_api__district_voter_density.sql` (mart → loaded to Postgres)

Join voters to their districts (the existing `m_people_api__districtvoter`
bridge — the same one that feeds people-api's `DistrictVoter`), group by
`district_id + h3`, k-suppress, and emit the cell centroid.
...
with district_voter as (
  ...
  from {{ ref('m_people_api__districtvoter') }} dv
...
exploded as (
  -- Unpivot the per-resolution h3 columns from int__people_api__voter_h3 into
  ...
  join {{ ref('int__people_api__voter_h3') }} vh on vh.lalvoterid = dv.voter_id  -- adapt join key
...
### 3.3 `m_people_api__district_voter_density_meta.sql` (mart → loaded to Postgres)
...
rendered as (
  ...
  from {{ ref('m_people_api__district_voter_density') }}   -- post-suppression
...
(`election-api.District.id == people-api.District.id`). **Reuse, never mint.**
...
Load the two marts into the people-api Postgres **after** districts, so the FK
target rows exist.

- Add `load_district_voter_density()` (and `load_district_voter_density_meta()`)
  to `airflow/astro/dags/load_people_api.py`, sequenced **after
  `load_districts()`** (FK ordering: `DistrictVoterDensity.district_id` →
  `District.id`).
- If the density row count is large enough that the bespoke loader is slow,
  instead register the two marts in the `people-api-loader`'s `_MART_MODELS`
  list and let the generic loader handle them (same as other marts).
- Cadence: `@daily`, matching the other people-api marts.
...
## 7. Postgres table contract (source of truth — matches people-api Prisma)

people-api owns these tables in the **`green`** schema. Column names/types below
are exactly what the app reads; the loader must write this shape. (`State` uses
the existing `green."USState"` enum, same as `DistrictVoter`.)
...
| `State`       | `green."USState"` enum  | two-letter state                             |
...
| `State`            | `green."USState"` enum |                                               |
| `updated_at`       | `timestamp`            |                                               |
| **PK**             | `(district_id, resolution)` |                                          |

## 8. What the app already does (for your validation)

- `GET /v1/people/voter-density?districtId=<uuid>&resolution=<int>` (people-api,
  S2S-only): reads `DistrictVoterDensity` (+ `DistrictVoterDensityMeta`) for the
  district/resolution and returns
  `{ districtId, resolution, coverage, minCellCount, cells: [{ lat, lng, count }] }`.
  No H3 math in Postgres — it just returns the precomputed centroids. `resolution`
  defaults to **8** when omitted.
- gp-api resolves `personId → Position.districtId` (via election-api) and proxies
