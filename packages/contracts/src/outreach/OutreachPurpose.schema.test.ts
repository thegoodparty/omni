...
describe('outreach purpose vocabulary', () => {
  ...
  it('keeps serve on its own list with the shared slugs renamed', () => {
    expect(SERVE_PHONE_BANKING_PURPOSE_VALUES).toContain('introduce_myself')
    expect(SERVE_PHONE_BANKING_PURPOSE_VALUES).toContain('event_invite')
    expect(SERVE_PHONE_BANKING_PURPOSE_VALUES).not.toContain('early_voting')
  })
