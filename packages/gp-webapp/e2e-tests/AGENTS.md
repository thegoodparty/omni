...
- **Flag-gated surfaces: force the flag, don't depend on Amplitude.** A test that exercises a feature behind an Amplitude flag must NOT rely on that flag's live targeting/rollout for the synthetic `@test.goodparty.org` user — that's flaky and couples the test to experiment config. Resolution is server-side (gp-api → Amplitude; the browser never calls Amplitude), so force the flag deterministically via the override-cookie seam: call `setFlagOverrides(page, { 'my-flag': 'on' })` (`src/helpers/campaignStory.helper.ts`) or a per-flag wrapper like `enableCrmFlags` (`src/helpers/crm-contacts-e2e.ts`) **before** auth/navigation, so the first SSR render already sees it. It's honored off-prod only (gated on `VERCEL_ENV`); there is no client SDK to stub. Mechanism + safety: `docs/feature-flags.md` § E2E overrides and `app/shared/experiments/flagOverrides.ts`.
- **Win CRM is unconditional; Serve CRM still drives on a flag** (ENG-10756, ENG-11009). `win-crm` hit 100% and was removed — the Win-persona contacts specs need no override cookie at all (`win-crm-default-on.spec.ts` pins that contract). `serve-crm` is still dark and ramping, so the Serve-persona specs (`tests/app/contacts/`, plus `contacts-org-scoping.spec.ts`) still force it **on** via the override cookie (`enableCrmFlags` in `src/helpers/crm-contacts-e2e.ts`) and assert against the rebuilt UI: no member table exists by design — filter correctness triangulates through the wizard's live count, the list-detail demographics, and typeahead-opened person records. `contacts-legacy-smoke.spec.ts` is the ONE retained flag-off spec (Serve, `serve-crm` pinned **off** via `disableCrmFlags`), covering the legacy page until it's deleted.
...
- **Wait for the specific query you're about to read, not for "a response".** `applyContactsQuery` (`src/helpers/contacts-e2e.ts`) wraps an action that re-queries the contacts table and resolves once the list response has landed AND the table skeleton has cleared. Both gates need the wait to be pinned to the right request: `ContactsTableProvider` runs a **page+1 prefetch** alongside the query the UI renders, so an unqualified `waitForResponse(/\/v1\/contacts/)` routinely resolves on the prefetch for the segment you're navigating _away_ from — which lands before the router navigation has even committed, so the skeleton check then passes against the pre-action DOM and the helper returns having observed the **old** segment throughout. The matcher therefore always requires `page=1` (every action these helpers wrap resets to page 1), and callers that know which segment they're selecting must pass it: `applyContactsQuery(page, action, { segment: 'doorKnocking' })`. Segment values come from `crm/configs/defaultSegments.config.ts`, not the option label.

  **Why it matters more than it looks:** auto-retrying assertions (`expect(locator).toHaveText`, `toHaveCount`) absorb an early return by polling. A **one-shot read of rendered text into a variable** does not — it silently captures the previous segment's value, and the test then compares a number against itself. That was the `door-knocking-household-dedupe` flake. Note that a `'--'`/empty-state guard does **not** protect such a read after a state change: `--` is a genuine pre-settled sentinel on first load only, since after a segment switch the pre-settled state is the previous segment's number. If you're snapshotting rendered text into a variable, the wait in front of it is the whole test.
