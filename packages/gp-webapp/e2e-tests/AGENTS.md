...
- **`@dev-only` (merge-only) tests.** The tag exists for ONE reason: a test that genuinely **cannot** run against an ephemeral per-PR preview because of an **infrastructure** dependency the preview can't stand up or reach — specifically an **inbound** dependency the preview can't receive. The bar is narrow; qualifying shapes are:
  ...
  **The tag's cost is a pre-merge blind spot.** A PR that changes a surface a tagged spec asserts on merges green and then turns the release train red — PR #1773 did exactly that for two days (ENG-11106). Two guards exist: the `devonly-drift` workflow posts an advisory PR comment when a diff's changed routes overlap tagged specs, and a flag default-on/removal PR must run the tagged suite against dev with the flag forced first (`docs/feature-flags.md` § Removing the flag). When the advisory names a spec, read it against your change before merging.
  ...
  **These are NOT valid reasons — they are the common mis-tags. Do not tag for any of them:**
  ...
  - **Real district voter data.** A per-PR preview's gp-api runs on the `GP_API_DEV` secret, so it serves the same real voter data as dev.
  ...
  - **The _consequences_ of an inbound webhook.** The inbound delivery earns the tag; what the handler then writes does not, and the two are separable. gp-api's route-test harness (`useTestService`, real Nest app + real Postgres, runs in the ordinary vitest suite and therefore on PRs) can generate a valid `stripe-signature` with the same `STRIPE_WEBSOCKET_SECRET` the app verifies against, so a correctly-shaped event goes through the real `POST /v1/payments/events` — parse, dispatch, handler, database — with no Stripe and no test-only endpoint. `src/payments/paymentEvents.cancellation.routes.test.ts` covers Pro cancellation that way. It also reaches effects a browser cannot see at all: the `PRO PLAN CANCELLATION` Slack notification, whose month-long absence in production was the whole incident, is a spy there and is invisible from Playwright. So reach for a gp-api route test before tagging a spec, and reserve the tag for the leg that genuinely needs the delivery (driving Stripe's own hosted UI, or asserting that Stripe really calls us).
...
- **Provision through the API before the browser holds a Clerk session. Never the other way round.** gp-api has no user row for a freshly created Clerk identity; the first authenticated request JIT-provisions it, and to do that `SessionGuard` asks Clerk's **Backend API** who the `clerkId` belongs to. That Backend API has a per-instance rate limit shared by every PR preview and every CI shard. The moment a browser has a Clerk session for an unprovisioned user, the app fans out ~8 concurrent authenticated calls (`/v1/campaigns/mine`, `/v1/organizations`, `/v1/experiment/variants`, …), **each** racing to provision the same identity through that budget. Clerk answers `429`, gp-api turns the failed lookup into a bare **`401`**, and the whole fan-out fails — including the setup call the test was waiting on. The browser fan-out has no retry, so it stays down while the budget is exhausted.

  That budget is per Clerk **instance**, not per run: every concurrent E2E run (another PR's shards, the release train, a re-run), gp-api dev's own lookups, and the test-user sweep spend from the same 100 req/10s. `tests/utils/throttle-requests-with-retry.ts` therefore budgets a whole CI run (all 4 shards x 4 workers) at half the instance limit, and `withGatewayRetry` backs off long enough (~50s) to outlast another run's burst (ENG-11105). If the 401 signature returns, suspect a third concurrent consumer before touching the throttle.
...
- **Flag-gated surfaces: force the flag, don't depend on Amplitude.** A test that exercises a feature behind an Amplitude flag must NOT rely on that flag's live targeting/rollout for the synthetic `@test.goodparty.org` user — that's flaky and couples the test to experiment config. Resolution is server-side (gp-api → Amplitude; the browser never calls Amplitude), so force the flag deterministically via the override-cookie seam: call `setFlagOverrides(page, { 'my-flag': 'on' })` (`src/helpers/campaignStory.helper.ts`) **before** auth/navigation, so the first SSR render already sees it. It's honored off-prod only (gated on `VERCEL_ENV`); there is no client SDK to stub. Mechanism + safety: `docs/feature-flags.md` § E2E overrides and `app/shared/experiments/flagOverrides.ts`.
- **Contacts is one unconditional surface for both personas** (ENG-10756, ENG-11009, ENG-11010). No flag gates it any more, so the contacts specs (`tests/app/contacts/`, plus `contacts-org-scoping.spec.ts`) need no override cookie to reach the CRM page. They assert against the rebuilt UI: no member table exists by design — filter correctness triangulates through the wizard's live count, the list-detail demographics, and typeahead-opened person records.
...
- **Wait for the specific query you're about to read, not for "a response".** An unqualified `waitForResponse(/\/v1\/contacts/)` can resolve on a request other than the one whose data you are about to assert on — including one still in flight for the state you are navigating _away_ from, which lands before the new state has even committed. Pin the wait to the request that produced the data: match the query parameters that identify it, and where a click re-keys a query, wait for the URL to carry the new value before the response wait resolves. Segment values come from `crm/configs/defaultSegments.config.ts`, not the option label.

  **Why it matters more than it looks:** auto-retrying assertions (`expect(locator).toHaveText`, `toHaveCount`) absorb an early return by polling. A **one-shot read of rendered text into a variable** does not — it silently captures the pre-change value, and the test then compares a number against itself. That was the `door-knocking-household-dedupe` flake. Note that a `'--'`/empty-state guard does **not** protect such a read after a state change: `--` is a genuine pre-settled sentinel on first load only, since after a change the pre-settled state is the previous value. If you're snapshotting rendered text into a variable, the wait in front of it is the whole test.
