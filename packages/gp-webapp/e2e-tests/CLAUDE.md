...
- **A Pro Win campaign is only reachable through Stripe.** `campaign.isPro` is written solely by the `checkout.session.completed` webhook — no candidate- or test-facing API can set it, so a fresh `authenticateTestUser` campaign is always non-Pro and every Pro-gated Win surface (e.g. the CRM create-list wizard) shows the upsell modal instead. To provision one, seed the pre-payment wizard state via API (`seedEinAndFiled` + `seedFilingComplete` + `seedCandidateProfileComplete`) then run `upgradeCampaignToProViaStripe` (all in `src/helpers/pro-upgrade.helper.ts`) — it lands on `/payment` directly, pays with the Stripe test card, and polls until the webhook flips `isPro` (up to ~240s; budget the test ≥15 min and tag it `@dev-only`, since previews don't receive Stripe webhooks).
