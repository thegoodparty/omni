...
// The campaign-story flag is resolved server-side (gp-api -> Amplitude) and
// seeded into the client, so there is no browser network call to stub. The
// `ff-overrides` cookie seam in getFlagVariants is the deterministic lever:
// enableCampaignStoryFlag(page) forces `campaign-story: on` for the context.
// That keeps these tests independent of Amplitude targeting, so they run on
// every PR (no @dev-only) rather than only against the dev stack.
...
const STORY_ANSWERS = {
  ...
  issues:
    'Fixing the streets, funding after-school programs, and keeping the budget transparent so residents can see where their money goes.',
} as const
...
test.describe('campaign-story flag flow', () => {
  ...
  test('campaign plan tab gates an incomplete story behind a link to it', async ({
    ...
    // Enable before auth: sign-in navigates and mounts the flag provider, so
    // the Amplitude stub must be live before its first fetch goes out.
    await enableCampaignStoryFlag(page)
  ...
  test('completing the story unlocks generation without enqueuing a job', async ({
    ...
    // Enable before auth: the Amplitude stub must be live before sign-in
    // navigates and the flag provider makes its first fetch.
    await enableCampaignStoryFlag(page)
    await authenticateTestUser(page, { isolated: true })
    ...
    // No footer until all three prompts have content.
    ...
    await fillStoryCard(page, 'issues', STORY_ANSWERS.issues)
    ...
    // The plan gate now shows the completed-story review with the three
    // answers and a real generate CTA.
    await expect(
    ...
    await expect(page.getByText(STORY_ANSWERS.issues).first()).toBeVisible()
