...
| `v2/OutreachDetailsDrawer.tsx` | Row-click drawer over `GET /v1/outreach/:id`. Scheduled-not-sent SMS rows while verification pends (threaded `tcrCompliance`) show a no-background destructive "Needs compliance" status, the compliance banner (Start compliance → the status-aware verification route), and a Cancel (the cancel confirm/endpoint) + Start verification footer; canceled rows swap the footer to Delete (`DELETE /v1/outreach/:id`, canceled-only server-side); paid-flow SMS rows (`phoneListId != null`) add a Payment details section (Total cost = `GET /v1/outreach/:id/receipt` amount in dollars, falling back to billable count × per-text price — 404 means a free row and reads "Free" — plus a View receipt link when the receipt exists) and a Message box rendering the stored composed script verbatim (pre-wrap, never re-composed). The same "Needs compliance" substitution runs in `v2/OutreachHistoryTable.tsx` (own filter entry; filtering operates on the displayed label). Also: overview metrics, audience chips, and for social the persisted per-platform assets with re-copy. For `nativePhoneBanking`, branches on the row's status: in-progress renders Progress ("X of N reached" off `phoneBanking.peopleCalled`/`peopleTotal`) + a static Payment details block (phone banking is free) + a "Continue calling" footer link to `/dashboard/outreach/phone-banking/[listId]`; completed renders one combined Results table (call-outcome breakdown from `byOutcome`/`entriesCalled`, then a yes/unsure/no support breakdown from `supporters`/`unsure`/`nonSupporters` over `peopleCalled`) + a footer pairing a compact ghost destructive Delete (`DELETE /v1/phone-banking/lists/:id`, cascades to the envelope row) with a `flex-1` outline Move to archive / Restore from archive (`PATCH /v1/outreach/:id/archive`, toggles off `row.archivedAt`) |
