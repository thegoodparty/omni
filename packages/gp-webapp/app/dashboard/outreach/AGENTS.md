...
| `v2/sms/SmsFlow.tsx` | The SMS channel flow (phase 2, initial build): purpose → audience (saved-list picker + in-flow list builder reusing the CRM wizard's `VoterFileStep`/`NameStep`/`useListWizardCount`; creates via the wizard's own endpoint, never edits an existing list) → schedule (48h min, 9am-9pm, no Send now) → compose → review+pay. Reuses the legacy draft-first purchase sequence verbatim: audience advance creates the Peerly phone list (`createP2pPhoneList` + `LongPoll` status), review entry creates the outreach with `draft: true`, and payment runs through `CheckoutSessionProvider`/`CheckoutPayment` (custom-mode Checkout Session, `PurchaseType.TEXT`). The embedded payment UI is themed to the design system via Stripe's Appearance API in `purchase/components/CheckoutPayment.tsx` — shared by every custom-session consumer (robocall inherits it), with BNPL pinned off server-side (`packages/gp-api/src/payments/AGENTS.md`) |
| `v2/sms/SmsComposeStep.tsx` | Compose with two system-owned regions framing the editable message: a greeting pill rendering Peerly's `Hello {first_name},` merge token (`SMS_GREETING` — the same token our 10DLC registration samples already submit, so the vendor contract assumes it) and the "Reply STOP to opt out." footer. The tone-flavored identification is NOT a locked region: it is the first sentence of the EDITABLE message — fresh AI drafts are prepended with it, and `hasIdentification` warns (and blocks the CTA) when an edit removes it. The AI body comes from `POST /v1/outreach/sms/draft` (per-tone memory / Improve / dictation, same contract as social; the server grounds drafts in the campaign story, issue positions, and plan — see `packages/gp-api/src/outreach/AGENTS.md`). The submitted script is the client-side concatenation greeting + message + footer (`composeScript`) — the backend has no region concept and sends the script to Peerly verbatim, merge token included. Image REQUIRED (500 KB, JPG/PNG/GIF): the backend rejects imageless text/p2p sends |
| `v2/sms/SmsReviewStep.tsx` | Review & pay: campaign summary card (date/time, audience + excluded counts, price, total), toggleable message-preview bubble of the composed script, the embedded payment card with the "$X due today" alert — and the free-texts zero-amount branch (`isFree` → `completeFreePurchase`, no payment UI) |
| `v2/sms/smsCompose.util.ts` | SMS purposes (the social slugs minus issue_update), `SMS_GREETING`, `identificationIntro` + `hasIdentification`, `composeScript`, image constraints |
...
- `packages/gp-api/src/outreach/AGENTS.md` — the backend module: draft-first purchase finalization, the compose endpoints and their campaign-materials grounding, and the Peerly seams.
