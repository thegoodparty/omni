...
- **A `nativeDoorKnocking` row's envelope holds the route's id and nothing else about the walk.** `OutreachDetailSchema` has no `doorKnocking` block, so doors, knockable people and logged progress are simply not here — the drawer names where they live rather than printing a People cell that can only ever say "—". Adding one means a contracts + gp-api change, not a client fix.
- **The archive seam: a door-knocking row is the projection, and this drawer must not write it.** `DoorKnockingTurf.archivedAt` (#1375) and `Outreach.archivedAt` are two flags for one walk, and only the door-knocking surface can keep both in step — the envelope carries `doorKnockingRouteId`, so turf → route → envelope resolves, while nothing maps a route id back to its turf. An archive button here could reach the envelope alone, which is precisely how the two come apart, so the `done` footer for door knocking carries a line pointing at Door knocking instead. `useTurfLifecycle` (`door-knocking/native/turfLifecycle.ts`) is the one writer, mirroring inside the archive mutation both the rail card and the list details drawer already call; the durable fix is mirroring inside gp-api's `setArchived`, the way `complete` already does.
