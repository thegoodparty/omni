...
| `v2/phone-banking/WhoStep.tsx` | The saved-list-vs-inline-audience picker described above; a failed/timed-out count (no server-side fenced-count retry — people-db's 25s statement timeout is the only bound) renders an error state with Try again, never an indefinite spinner. Reports the saved-list count's pending/failed state up to `PhoneBankingFlow` via `onCountStatusChange` so Continue can't advance past a count it knows failed — the inline-audience count has no such gate; its failure is caught at create time by the API's validation instead |
