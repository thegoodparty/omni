              ...
              <Table>
                ...
                <TableBody>
                  {detailQuery.data.outreachHistory.map((entry) => {
                    // ENG-10776: legacy rows can carry a null `date` (e.g. a
                    // hand-logged draft) — createdAt is always present, so
                    // the row never renders a bare "—" date.
                      ...
                      <TableRow key={entry.id}>
                        ...
                        <TableCell className="text-muted-foreground">
                          <Badge variant="soft" shape="pill">
                            {/* ?? guards the deploy-skew window where the
                                API serves a channel newer than this
                                bundle's map. */}
