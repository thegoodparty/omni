...
- **CRM typeahead search** (ENG-10687): when `useCrmEnabled()` (app/dashboard/shared) is on, `ContactsPage` swaps `ContactSearch` for `ContactTypeahead` — a debounced (300ms, min 3 chars) results dropdown over the same `GET /v1/contacts` client path (`resultsPerPage: 8`), fetched standalone via `useContactTypeaheadSearch` (query key `['contacts-typeahead', orgSlug, term]`, never the table's `['contacts', ...]` key or the `query` URL param); selecting a row navigates via the provider's `selectPerson`. Flag off or unsettled renders today's `ContactSearch` unchanged. Non-pro: focus/typing opens the Pro modal and no request fires (gp-api still rejects non-pro search). The ENG-10688 analytics seam is the `settled` derivation inside `useContactTypeaheadSearch`.
