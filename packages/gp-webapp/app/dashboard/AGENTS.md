...
| `shared/membership/` | Membership status surfaces behind `outreach-pro-gating-v2`: `MembershipBanner` in the sidebar footer (desktop, rendered by `DashboardMenu`), `MembershipChip` in the mobile top bar (rendered by `DashboardLayout`'s `MobileMenuTrigger`) — both fire the flag's exposure, and only once they are actually visible, plus the `ProPitchDialog` and `PinDialog` they open. One state (`deriveMembershipState` / `useMembershipState`) drives both: free → the Pro pitch, needs verification → `/dashboard/campaign-verification`, in review → no action, awaiting PIN → the PIN dialog. Hidden for Serve (elected office) and for cleared Pro campaigns |
...
- **Sidebar visibility** is driven by the menu config in `DashboardMenu.tsx` — adding a feature route means adding a menu entry there too. Adding a menu entry ALSO means adding the tab to the AI assistants' product map (`packages/gp-api/src/chats/general/product-knowledge/productMap.ts`), which is how the Campaign Manager and Chief of Staff answer "where do I find X" instead of guessing at a menu they cannot see. CI fails without it; a pre-commit check and a Claude Code hook catch it first. Anything user-reachable that never becomes a tab needs an entry too, and no check can see that one. See `docs/product-knowledge.md`.
