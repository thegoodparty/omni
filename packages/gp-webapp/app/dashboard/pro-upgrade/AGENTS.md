...
| File                                             | Role                                                                                                                                                                                               |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `page.tsx` → `components/ProUpgradeEntry.tsx`    | Wizard **index**. Reads the canonical-state queries, calls `deriveProUpgradeStep`, `router.replace`s to the resume step (propagating `?src`). Shows a recoverable error (not a mis-route) if any query fails.           |
| `layout.tsx` → `components/ProUpgradeWizard.tsx` | Wizard **shell**, now TWO chromes. Legacy (default): Exit link, desktop vertical `Stepper`, the 640px card. Takeover (`?src=outreach`, Voter Outreach 2.0 parity 2026-08-27): the design's full-screen chrome — "Upgrade to Pro" eyebrow + `Progress` bar header, 672px column, corner close, pinned footer. Every in-wizard navigation propagates `src` (`proUpgradeStepPath(step, src)`), so the fork survives step changes; the Stripe 3DS fallback return URL loses it (lands on legacy chrome — accepted). Un-forking later = defaulting `takeover` on. Provides `useProUpgradeWizard()` (`currentStep` / `goToStep` / `goToNextStep` / `goToPreviousStep`). |
| `components/WizardStepFooter.tsx` + `components/WizardHeading.tsx` | The per-step footer/heading, one component each with two presentations: under the takeover chrome the footer PORTALS into the shell's pinned bar (design row: primary right min-w-240, ghost Back left min-w-140; mobile stacked primary-on-top) and the heading renders the design intro (ProBadge+lock row, 20px title, 16px muted subtitle); outside it they render the exact legacy inline markup. Steps must use these — never hand-roll a footer div again, or the takeover chrome loses that step's CTA. The takeover slot machinery lives in `app/dashboard/shared/takeover/TakeoverShell.tsx`. |
