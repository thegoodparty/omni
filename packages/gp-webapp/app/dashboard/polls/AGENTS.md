...
- **Polls blocks entirely when the org has no constituent data.** Polls is Serve-only and Serve has no Pro gate (`canUseProFeatures` is always true for an `eo-` org), so this gate is the only protection. `calculateRecommendedPollSize` was turning the missing total into `NaN` via `MAX_CONSTITUENTS_PER_RUN - undefined` — straight into the audience options and cost preview. A poll against an unknown audience can't be priced, so the product call was to block: `polls/onboarding/OnboardingPage` renders a terminal state (mirroring `NotEnoughConstituents`) and both `CreatePoll` and `ExpandPollPage` branch on `useTotalConstituentsWithCellPhone().isUnavailable`. All of them route to `/dashboard/contacts`, which already explains the problem and offers support — polls deliberately does not repeat the handoff.
- **"Unavailable" is the request's outcome, not a prediction.** There are two ways `GET /v1/contacts/stats` can fail this way and `useDistrictResolution`'s `district === null` predicate only models one: an org with no resolvable district (~74 of 1,879 Serve orgs, mostly write-in office names), **and** a district that resolves but has no `DistrictStats` row (51 of 1,665 Serve districts). Both return 400 + `VOTER_DATA_UNAVAILABLE`; only the second is invisible to the predicate, and those orgs sailed past the gate into the `NaN` flow. So `isUnavailable` is `isUnresolvable || query.isError`, and the predicate survives *only* as an `enabled` flag that skips a call we already know is doomed. Don't reintroduce a gate that branches on `isUnresolvable` alone. `districtStatsQueryOptions` sets `retry: false` for the same reason — both states are permanent for the org, so the global `retry: 2` just triples a doomed request.
