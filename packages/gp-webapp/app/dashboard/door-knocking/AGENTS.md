...
- **Stops, doors and people are three different numbers — `routeCounts.ts` owns the definitions.** A stop is a geocoded coordinate the router visits, a door is an address (unit included), a person is a targeted voter; a block of flats is one stop, many doors, more people. Anything user-facing that says "doors" must count doors: `countDoors`/`countPeople` for a frozen route, `polygonStats().households` for a shape being drawn. This drifted badly once — `WalkView` printed `stopCount` as doors, `WalkSheet` summed targets as doors, and the same route reported two different door counts depending on the surface. The 150 cap is on **stops**, so a "150-stop limit" message next to a doors count is correct, not a mismatch.
