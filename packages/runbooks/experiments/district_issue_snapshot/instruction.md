...
- **`aligned_voter_percentage` is in the 30–80 range.** If it's 0–5%, first check whether the matched column is in the ~106 columns that are null in most states (two vendor vintages cover only 12 or 39 states respectively) — an all-null column scores 0% with the correct `>= 50` threshold; if so, set `matched_hs_column = None` and re-run without that column. If coverage is not the issue, you likely used `= 1` instead of `>= 50` (binary inference from suffix) — re-do Step 3 with `>= 50`. If it's >95%, you matched the wrong column or your sample is too narrow.
...
| `aligned_voter_percentage` is 0-5% | Matched column is null in this state (12-/39-state vendor vintages — 0% with a correct query), or `= 1` used instead of `>= 50` | Check coverage first (`COUNT(col)`); if null, set `matched_hs_column = None` and re-run without it. Otherwise re-do Step 3 with `>= 50` |
