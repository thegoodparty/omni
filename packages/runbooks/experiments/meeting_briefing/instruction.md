...
1. **Primary platform** (try in order; each requires its own search query + verification fetch):
   ...
   - CivicWeb (GHD Digital / iCompass): WebSearch `"<city>" civicweb` → `{client}.civicweb.net`. Meetings feed `GET /Services/MeetingsService.svc/meetings?from=<date>&to=9999-12-31` (plain JSON, filter by `MeetingDate` → `Id`), then `/Portal/MeetingInformation.aspx?Id={Id}` for `/document/{docId}` links; PDF at `/document/{docId}?printPdf=true`. Never enumerate document IDs. (See the CivicWeb entry in the platform reference below.)
...
- **CivicWeb** (GHD Digital / iCompass) — `https://{client}.civicweb.net`, `{client}` = subdomain (e.g. `monroewa`, `threeriversmi`). Resolve date→agenda through the portal's own JSON meetings feed — **never probe sequential `/document/{id}` IDs.** `GET /Services/MeetingsService.svc/meetings?from=YYYY-MM-DD&to=9999-12-31` returns a plain JSON array (no auth); each entry has `Id`, `MeetingDate`, `Name`, `TypeId` (the body), and `Published` (is the agenda posted yet?). Filter by `MeetingDate` for the meeting's `Id`, then `GET /Portal/MeetingInformation.aspx?Id={Id}` to reach its `/document/{docId}` links (typically an Agenda plus a fuller Agenda Packet). Download a packet as PDF with `GET /document/{docId}?printPdf=true` (bare `/document/{docId}` is an HTML viewer, not the PDF). The `.svc` feed and the `?printPdf=true` PDF work with plain `http.get`/`pdf.download`; the calendar, meetings list, and `MeetingInformation.aspx` pages are JS-rendered (a plain fetch sees only a "Loading…" placeholder), so trust the feed's `Published` flag rather than scraping those pages. The old `/Api/OpenData/v1/...` OData path does NOT exist (404). `/filepro/documents/{folderId}` is the browsable Document Center (folders by body) for archives if the feed misses.
