...
- **All sentiment percentages <5%** → you used `= 1` instead of treating `hs_*` as 0-100 scores. Re-do the distribution check. (A NULL `mean_score` is a different case: the column has no coverage in this state — vendor vintage — not an error; null the section with `haystaq_status: "no_column"` per Step 16, never coerce it to 0.)
