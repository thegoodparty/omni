...
| `ready_to_submit`                | Website published and live; not yet submitted to Peerly                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Steps 2 to 5 |
| `awaiting_pin`                   | Website published and live, and the registration is **already submitted** to Peerly — a CampaignVerify PIN is outstanding. Skip Steps 2 to 5, then still run Step 6: the submit tool is idempotent and returns the existing registration (with its `peerly_request_id`) without re-submitting. Running it rather than assuming completion is deliberate — it is also what keeps this row correct during the minutes after a release when gp-api may still be reporting `awaiting_pin` for a record that has **not** been submitted yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Steps 2 to 5 |
| `filing_review_hold`             | gp-api's pre-submission check is holding this record: the saved filing URL could not be verified as an official election-authority filing for this candidate. A submission would be rejected by CampaignVerify, so do not attempt one. Clearing it is an operator action (correct the filing data, or apply a staff override) that you cannot do and must not try. Immediately set `stage: "failed"`, append blocker `{ step: "submit_tcr", code: "peerly_rejection", detail: "gp-api is holding this record on CV pre-submission validation of the filing URL", first_seen_at: <ISO now>, retry_count: 0, is_recoverable: false }`, and go to Step 7.                                                                                                                                                                                                                                                                                                                                                                             | Steps 2 to 6 |
| `tcr_in_review` / `tcr_approved` | Already submitted to Peerly on a live site                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Steps 2 to 6 |
| `tcr_rejected`                   | The record still carries an **unresolved** rejection: a prior submission was rejected and nobody has cleared it yet. Do not resubmit — the data that caused the rejection is still on the record, so a resubmission re-fails deterministically and spams CampaignVerify. This is not a verdict that the candidate is permanently unregistrable: clearing it is an operator action (correct the filing data, then reset `tcr_compliance.status` off `rejected`/`error` so the stage leaves `tcr_rejected`), documented in `books/recover-rejected-10dlc-compliance.md`. You cannot do that yourself and must not try. Immediately set `stage: "failed"`, append blocker `{ step: "submit_tcr", code: "peerly_rejection", detail: "", first_seen_at: <ISO now>, retry_count: 0, is_recoverable: false }` — `detail` is `""` because the compliance-state response carries no rejection reason — and go to Step 7. Once an operator has cleared the rejection, a re-dispatch sees `ready_to_submit` here instead and submits normally. | Steps 2 to 6 |
...
| `ready_to_submit` / `awaiting_pin` | `website_verified_live`             | Steps 2-5 done; Step 6 (submit TCR) is next      |
...
- **No match at `initial_cap`, `domain_budget_cap_usd > 10`, and the response says `partial: false`.** (If `partial` is true, the empty list is a quota artifact, not a price problem — use the `partial: true` bullet below instead and do **not** escalate.) Re-call **once** with `price_cap_usd = domain_budget_cap_usd` (the explicit higher override). **The pre-call guard above applies to this call as well** — before sending, confirm the request still satisfies all three conditions (full pattern catalog, all six approved TLDs, randomized order); if not, append the `pattern_catalog_incomplete` blocker, set `stage: "failed"`, and go to Step 7. If a match appears, first confirm the returned domain's TLD is in the allowlist (`run`, `bio`, `fyi`, `win`, `digital`, `site`). If it is not, append blocker `{ step: "domain_search", code: "unapproved_tld_returned", detail: <the returned domain>, first_seen_at: <ISO>, retry_count: 0, is_recoverable: false }`, set `stage: "failed"`, and go to Step 7 — do **not** purchase. Otherwise proceed to Step 3 with that name + price.
- **No match, but the response says `partial: true`.** The list is a floor: some candidates went unchecked because the registrar rate limited us or the search hit its time budget. That is not evidence the catalog is taken — do **not** escalate the price cap and do **not** record `budget_exceeded`. Wait 30 seconds and re-call **once** at the same cap. If the retry still returns no match with `partial: true`, append blocker `{ step: "domain_search", code: "registrar_rate_limited", detail: "", first_seen_at: <ISO>, retry_count: 1, is_recoverable: true }`, leave `stage` at `domain_search_started`, and go to Step 7 — the recovery loop will re-dispatch the run.
- **No match within budget** (either `initial_cap == domain_budget_cap_usd` so no escalation is possible, or the escalated retry also returned nothing), and the response says `partial: false` so every candidate was actually checked. Append blocker `{ step: "domain_search", code: "budget_exceeded", detail: "", first_seen_at: <ISO>, retry_count: 0, is_recoverable: false }`. Set `stage: "failed"`. Go to Step 7.
...
- **Never write `stage: "tcr_submitted"` without an actual Peerly submission.** `tcr_submitted` is legitimate only when Step 6 succeeded _this run_ (`tcr_submission.peerly_request_id` is non-empty) OR you skipped Step 6 because the read stage was already `tcr_in_review`/`tcr_approved` (then `submit_tcr` is in `skipped_steps`). Reading `ready_to_submit` is **not** "already submitted" — it means exactly what it says: run Step 6. Reading `awaiting_pin` means the registration is already in, but you still run Step 6 to obtain the existing `peerly_request_id` idempotently rather than assuming it. The validator rejects `tcr_submitted` with an empty `peerly_request_id` and no skipped `submit_tcr`, so do not treat the run as done until you have actually called the submit tool.
