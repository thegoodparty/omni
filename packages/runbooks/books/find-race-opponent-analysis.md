...
- **The input side needs the same manifest change.** The `input_schema` is also `additionalProperties: false` and on the current manifest lists only `opponents` and `race_context`. ENG-10591 must add `candidate_platform` to its `properties` (optional, not `required`) in the same change, or the first dispatch carrying `candidate_platform` is rejected by the input validator. Update both schemas together, not just the output.
