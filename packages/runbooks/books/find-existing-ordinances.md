...
- **PDF handling.** The runtime's `http.get` refuses binary content; the instruction uses `pmf_runtime.http.download(url)` (returns `{path, byte_size, source_url, content_type}`) and extracts text with `pdftotext -layout` (poppler-utils ships in the runner image; `pip install` cannot reach PyPI from the quarantined container).
