...
**Credential handling:** the login bundle contains a session token and password.
Use them only to authenticate the browser against `TARGET_URL`. Never write them
into findings, summaries, or any output file.
...
- A **login bundle** per test user: the cookie triple (`token`, `user`,
  `organization-slug`) from the test-fixtures API, plus email/password as a
  fallback.
...
1. **Log in by cookie injection.** Navigate to `TARGET_URL` once (so the origin
   exists), then set the three cookies via `browser_evaluate`
   (`document.cookie = ...; path=/`), then reload. Verify you landed
   authenticated as the right identity: the dashboard renders and the org
   context matches the provided `organization-slug` (Serve fixtures land on
   `/dashboard/chief-of-staff`; Win fixtures on `/dashboard`). If cookie
   injection fails, try one password login through the real sign-in UI (the
