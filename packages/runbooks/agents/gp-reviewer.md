...
3. **Audit the tests against the diff — this is the single most-missed blocker class, run it every review.** Don't read the test file in isolation and nod; cross-check it against the new behavior. For **every new or changed branch/behavior** the diff introduces (a new `if`/`?:`/`.find()`/early-return, a new conditional like `threatTier === 'primary_threat'`, a dedup/once-guard like `Set.has`, a new field on a payload):
   - **Coverage:** find the test case whose *input actually reaches this branch*. A fixture that omits the field the branch keys on (`threatTier` unset → `.find()` always `undefined` → the fallback is all that ever runs), or a single-item fixture that never re-enters a dedup guard, means the new path has **zero coverage** → `blocker`. Grep the fixtures for the value that triggers the branch; if nothing sets it, the branch is unreachable in the suite.
   - **Assertion strength (disprove-it *on the test*):** for each assertion that is the only coverage on new behavior, ask "would this still pass if the implementation were wrong or deleted?" If yes, it's tautological → `blocker`. Concrete vacuous patterns to flag: `expect.objectContaining({})` or `expect.any(...)` as the *only* payload matcher (matches `{}`/`null`/a wrong object); asserting only `toHaveBeenCalled()`/`toHaveBeenCalledTimes(n)` without checking the produced value or side effect; **a shared mock reference reused across two stubs** so the assertion can't distinguish the correct path from a bypass — trace which exact mock object the code-under-test reads (`this.client.x` vs `this.dep.client.x`) and confirm the assertion is on *that* one.
4. For each candidate finding, run the **disprove-it pass** (below) before emitting.
...
- Two recurring triggers that look "theoretical" but are NOT — emit them as `blocker`s, don't disprove them away:
  - **Read-then-write without a transaction/atomic claim on a handler the platform calls concurrently by design.** Webhook handlers (Stripe et al.) and SQS consumers are at-least-once with retries — two concurrent deliveries for the same entity is *normal*, not adversarial timing. A `findUnique` then `update` (or a "first time becoming X?" check then a dispatch) with no wrapping `$transaction`/`Serializable` or atomic `updateMany WHERE …` means both callers read the old state and both fire the side effect. The fact that surrounding code is async does not serialize it.
  - **Input accepted at a boundary but silently dropped before persistence.** An endpoint/DTO accepts a field (a URL hint, an option) that gets passed to a service but never written to a column — so it's lost on any later re-read (a retry, a resume, a re-dispatch after `FAILED`). Trace each new request field through to the DB write; if it never lands, the feature silently discards it → `blocker`.
