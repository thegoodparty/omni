...
def get_agent_mcp_secret() -> str:  # pragma: no cover
    raise NotImplementedError("Must be overridden via dependency_overrides")


def get_agent_fleet_id() -> str:  # pragma: no cover
    raise NotImplementedError("Must be overridden via dependency_overrides")
...
async def proxy_mcp_root(
    ...
    secret: str = Depends(get_agent_mcp_secret),
    fleet_id: str = Depends(get_agent_fleet_id),
    base_url: str = Depends(get_gp_api_base_url),
    http: httpx.AsyncClient = Depends(get_http_client),
):
    if not ticket.clerk_user_id:
        raise HTTPException(
            status_code=500,
            detail={"reason": "ticket_missing_clerk_user_id"},
        )
