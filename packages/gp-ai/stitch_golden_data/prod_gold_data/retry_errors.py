...
class ErrorRetryProcessor:
    ...
    def find_parquet_files(self, filename: str | None = None) -> list[str]:
    ...
    def identify_error_records(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(error_records) > 0:
            ...
            quota_errors = (
                (error_records["l2_district_name"] == "QUOTA_ERROR").sum()
    ...
    async def retry_error_records(self, error_records: pd.DataFrame) -> pd.DataFrame:
        ...
        for _idx, row in error_records.iterrows():
            try:
                ...
                if "quota exhausted" in error_str or "resource_exhausted" in error_str:
                    self.logger.error("🚨 Quota exhausted during retry - stopping")
    ...
    def merge_retried_records(
        self, original_df: pd.DataFrame, error_records: pd.DataFrame, retried_records: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        for _idx, retried_row in retried_records.iterrows():
            ...
            if "id" in retried_row and "id" in updated_df.columns:
                ...
                match_mask = (updated_df["name"] == retried_row["name"]) & (updated_df["state"] == retried_row["state"])
    ...
    def save_updated_file(
        self, df: pd.DataFrame, original_filepath: str, fixed_row_indices: list[int], error_records: pd.DataFrame
    ):
        ...
        if os.path.exists(original_tsv_path):
            with open(original_tsv_path) as f:
                lines = f.readlines()
                capturing = False
                for line in lines:
        ...
        successfully_fixed = len(
            [
                idx
                for idx in fixed_row_indices
                if df.iloc[idx]["is_matched"] or "NOT_MATCHED" in str(df.iloc[idx]["llm_reason"])
            ]
        )

        # Get sample of fixed records for metadata
        fixed_samples = []
        for _i, idx in enumerate(fixed_row_indices[:5]):  # First 5 fixed records
            row = df.iloc[idx]
            district_name = row.get("l2_district_name", "Unknown")
            fixed_samples.append(
                f"#   Row {idx + 1}: {row['name']} -> {district_name if row['is_matched'] else 'NOT_MATCHED'}"
            )
        ...
        return {"parquet": fixed_parquet_path, "tsv": fixed_tsv_path}
    ...
    async def process_file(self, filepath: str):
        ...
        try:
            ...
            self.save_updated_file(updated_df, filepath, fixed_row_indices, error_records)
    ...
    def print_final_summary(self):
        ...
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("ERROR RETRY FINAL SUMMARY")
        self.logger.info(f"{'=' * 80}")
        self.logger.info(f"Total Error Records Found: {self.retry_stats.total_error_records:,}")
        self.logger.info(
            f"  - Processing Errors: {self.retry_stats.total_error_records - self.retry_stats.total_quota_errors:,}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--dry-run", "-d", action="store_true", help="Show what would be processed without making changes"
    )
