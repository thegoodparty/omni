...
class LLMSelection:
    ...
    alternative_matches: list[dict] | None = None
...
class ProductionMatcher:
    ...
    def __init__(
        self, catalog="goodparty_data_catalog", br_schema="dbt", br_table="int__enhanced_position", max_workers=1500
    ):
    ...
    def get_states_by_record_count(self, ascending: bool = True) -> list[str]:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Ordered {len(ordered_states)} states by record count ({'smallest first' if ascending else 'largest first'})"
            )
    ...
    def load_vector_store(self, state: str) -> dict | None:
    ...
    def load_br_data(
        self, states: list[str] | None = None, limit: int | None = None, sample_fraction: float | None = None
    ) -> pd.DataFrame:
        ...
        try:
            self.logger.info("🔄 Loading BR data from Databricks...")
            df = self.databricks.execute_query(query)
    ...
    async def llm_select_best_match(self, br_name: str, districts: list[EmbeddingDistrict]) -> LLMSelection | None:
        ...
        for i, district in enumerate(districts, 1):
            district_descriptions.append(f"{i}. {district.l2_district_name} ({district.l2_district_type})")
...
"""
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: self.llm.generate_structured_content(
                    prompt=prompt, response_schema=response_schema, trace_name="stitch-match-selection"
                ),
            )
        except Exception as e:
        ...
        try:
            ...
            if selected_number < 0 or selected_number > len(districts):
                self.logger.warning(
                    f"Selected candidate number {selected_number} out of bounds (0-{len(districts)}). Defaulting to 0 (no match)."
                )
                selected_number = 0
        except (ValueError, TypeError, KeyError):
            self.logger.warning(
                f"Invalid selected_candidate_number in LLM response: {response.get('selected_candidate_number', 'missing')}. Defaulting to 0 (no match)."
            )
            selected_number = 0
            ...
            for alt_num in response["close_alternatives"]:
                try:
                    ...
                    if 0 < alt_num_int <= len(districts):
                        ...
                        self.logger.warning(
                            f"Alternative match number {alt_num_int} out of bounds (1-{len(districts)}). Skipping."
                        )
        ...
        try:
            ...
            for district in embedding_districts:
                embedding_matches.append(
                    f"{district.l2_district_type} - {district.l2_district_name} ({district.similarity_score:.3f})"
                )
            ...
            if llm_selection.alternative_matches:
                alt_list = [
                    f"{alt['district_name']} ({alt['district_type']})" for alt in llm_selection.alternative_matches
                ]
    ...
    async def process_batch(
        self, batch_df: pd.DataFrame, batch_num: int, total_batches: int, state: str = None
    ) -> pd.DataFrame:
        ...
        self.logger.info(
            f"🔄 Batch {batch_num}/{total_batches}{state_info} ({progress_pct:.1f}%) - Processing {batch_size} records"
        )
        ...
        if batch_size > 0:
            ...
            completed_records = (
                ...
                else len(processed_rows) + ((batch_num - 1) * batch_size)
            )
    ...
    async def run_production_matching(
        self,
        states: list[str] | None = None,
        limit: int | None = None,
        batch_size: int = 1000,
        output_filename: str = "production_matching_results.tsv",
    ) -> str:
        """Run production matching on BR database"""

        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("PRODUCTION BR-L2 MATCHING")
        ...
        if states:
            ...
            self.logger.info("📊 Using natural order: smallest to largest states")
        ...
        for batch_start in range(0, len(br_df), batch_size):
            ...
            if batch_num % 10 == 0 or batch_num == total_batches:
                self.logger.info(
                    f"💰 Running cost: ${self.stats.total_cost:.6f} ({self.stats.total_processed:,} processed)"
                )
    ...
    async def process_single_state(self, state: str, batch_size: int, output_prefix: str) -> dict[str, Any]:
        ...
        if os.path.exists(parquet_path):
            ...
            return {
                "state": state,
                "status": "SKIPPED",
                "output_paths": {"parquet": parquet_path, "tsv": parquet_path.replace(".parquet", ".tsv")},
                "stats": MatchingStats(),
            }
        ...
        if br_df.empty:
            ...
            return {"state": state, "status": "NO_DATA", "output_paths": {}, "stats": state_stats}
        ...
        self.logger.info(
            f"   Matched: {state_stats.successful_matches:,}/{state_stats.total_processed:,} ({success_rate:.1f}%)"
        )
        self.logger.info(f"   Cost: ${state_stats.total_cost:.6f}")
        self.logger.info(f"{'=' * 60}\n")

        return {"state": state, "status": "COMPLETED", "output_paths": state_output_paths, "stats": state_stats}
    ...
    def save_enhanced_results_with_stats(
        self, results_df: pd.DataFrame, base_filename: str, stats: MatchingStats
    ) -> dict[str, str]:
        ...
        return {"parquet": parquet_path, "tsv": tsv_path}

    async def run_all_states_individual(
        self, batch_size: int = 1000, output_prefix: str = "state_matching", max_concurrent_states: int = 3
    ) -> dict[str, str]:
        """Run production matching for all available states with controlled concurrent processing"""

        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("PRODUCTION BR-L2 MATCHING - ALL STATES CONCURRENT")
        self.logger.info(f"{'=' * 80}")
        ...
        self.logger.info(
            f"📁 Processing {len(available_states)} states in size order (smallest first) with max {max_concurrent_states} concurrent"
        )
        ...
        async def process_state_with_semaphore(state: str) -> dict[str, Any]:
            async with semaphore:
                try:
                    ...
                    raise RuntimeError(f"State {state} processing timeout after 2 hours") from None
                except Exception as e:
            ...
            with tqdm(
                ...
                for state_idx, state in enumerate(available_states, 1):
                    try:
                        ...
                        error_str = str(e).lower()
                        if "daily quota exhausted" in error_str or "resource_exhausted" in error_str:
                            self.logger.error("🚨 QUOTA EXHAUSTED - stopping all state processing")
                            break
                        else:
            ...
            with tqdm(
                ...
                for completed_result in asyncio.as_completed(tasks):
                    try:
                        ...
                        error_str = str(e).lower()
                        if "daily quota exhausted" in error_str or "resource_exhausted" in error_str:
                            self.logger.error("🚨 QUOTA EXHAUSTED - cancelling all remaining state processing")
                            # Cancel all remaining tasks
                            for task in tasks:
                                if not task.done():
                                    task.cancel()
                            raise RuntimeError(f"Processing stopped due to quota exhaustion: {str(e)}") from e
                        else:
                            self.logger.error(f"❌ State processing error: {e}")
                            continue
    ...
    def print_final_all_states_summary(self, all_output_paths: dict[str, str], skipped_states: list[str]):
        ...
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("ALL STATES INDIVIDUAL MATCHING FINAL SUMMARY")
        self.logger.info(f"{'=' * 80}")
    ...
    def print_data_summary(self, df: pd.DataFrame):
        ...
        self.logger.info(
            f"Original BR columns: {len([col for col in df.columns if not col.startswith(('l2_', 'is_matched', 'llm_', 'confidence', 'top_embedding', 'embeddings', 'embedding_queried_term', 'alternative_matches'))])}"
        )
        self.logger.info(
            f"Added matching columns: {len([col for col in df.columns if col.startswith(('l2_', 'is_matched', 'llm_', 'confidence', 'top_embedding', 'embeddings', 'embedding_queried_term', 'alternative_matches'))])}"
        )
    ...
    def print_final_summary(self):
        ...
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("PRODUCTION MATCHING FINAL SUMMARY")
        self.logger.info(f"{'=' * 80}")
        self.logger.info(f"Total Records: {self.stats.total_processed:,}")
        self.logger.info(f"Successful Matches: {self.stats.successful_matches:,} ({success_rate:.1f}%)")
        self.logger.info(f"  - High Confidence (>80%): {self.stats.high_confidence:,}")
        ...
        if llm_stats["completion_tokens"] + llm_stats["thinking_tokens"] > 0:
            thinking_pct = (
                llm_stats["thinking_tokens"] / (llm_stats["completion_tokens"] + llm_stats["thinking_tokens"]) * 100
            )
...
async def main():
    ...
    parser.add_argument("--state", "-s", type=str, help="Process a single state (e.g., DE, CA, NY)")
    parser.add_argument("--states", nargs="+", type=str, help="Process multiple states (e.g., --states HI DE DC)")
    parser.add_argument("--batch-size", "-b", type=int, default=1000, help="Batch size for processing (default: 1000)")
    parser.add_argument("--limit", "-l", type=int, help="Limit number of records to process")
    parser.add_argument(
        "--max-concurrent-states", "-c", type=int, default=3, help="Maximum concurrent states to process (default: 3)"
    )
    parser.add_argument("--max-workers", "-w", type=int, default=1500, help="Maximum parallel workers (default: 1500)")
    ...
    if args.states:
        ...
        print(
            "   print(df[['name', 'state', 'embedding_queried_term', 'l2_district_name', 'is_matched', 'confidence']].head())"
        )

        print("\n💡 Usage examples:")
        print("   # Process Delaware only (high throughput):")
        print("   uv run stitch_golden_data/prod_gold_data/production_matcher.py --state DE")
        print("   # Process all states with 2 concurrent states:")
        print("   uv run stitch_golden_data/prod_gold_data/production_matcher.py all_states --max-concurrent-states 2")
        print("   # Process Delaware with custom batch size:")
        print("   uv run stitch_golden_data/prod_gold_data/production_matcher.py --state DE --batch-size 1000")
        print("   # Process all states with maximum throughput:")
        print(
            "   uv run stitch_golden_data/prod_gold_data/production_matcher.py all_states --batch-size 1000 --max-concurrent-states 2 --max-workers 1500"
        )
