...
class VectorStoreGenerator:
    ...
    def load_state_data(self, state: str, force_reload: bool = False) -> pd.DataFrame | None:
    ...
    def create_embedding_texts(self, df: pd.DataFrame, state: str) -> list[dict]:
        ...
        for _, row in df.iterrows():
            ...
            texts.append(text)
            metadata.append({"district_name": district_name, "district_type": district_type, "state": row["state"]})

        return texts, metadata

    async def generate_state_embeddings(
        self, state: str, batch_size: int = 100, force_regenerate: bool = False
    ...
    async def generate_all_states(
        self, batch_size: int = 100, force_regenerate: bool = False, states_to_process: list[str] | None = None
    ):
        """Generate vector stores for all 50 states + DC"""
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("GENERATING VECTOR STORES FOR ALL US STATES + DC")
    ...
    def print_generation_summary(self, results: list[StateProcessingResult], total_cost: float, total_districts: int):
        ...
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("VECTOR STORE GENERATION SUMMARY")
        self.logger.info(f"{'=' * 80}")
    ...
    def list_vector_stores(self):
        ...
        self.logger.info(f"\n{'=' * 80}")
        self.logger.info("AVAILABLE VECTOR STORES")
        self.logger.info(f"{'=' * 80}")

        vector_files = [
            f for f in os.listdir(self.vector_store_dir) if f.startswith("l2_embeddings_") and f.endswith(".pkl")
        ]
...
async def main():
    ...
    await generator.generate_all_states(
