...
class TestVerdictsAndDeflections:
    def test_only_no_code_change_is_a_deflection(self):
        facts = verdicts(REPORT_RUNS)

        assert facts["counts"] == {"fix": 3, "no-code-change": 3, "needs-human": 1}
        assert facts["deflected"] == 3

    def test_a_needs_human_week_deflected_nothing_and_still_shows_its_count(self):
        # `needs-human` is an escalation, not a deflection: the ticket still
        # reaches a person. A week of nothing but escalations kept nothing off
        # the queue, and the message has to say so while still reporting that
        # the escalations happened. Counting them was the 2026-09-14 digest's
        # "6 tickets kept off the eng queue" when three were on somebody's
        # plate that morning.
        runs = [a_run(verdict="needs-human"), a_run(verdict="needs-human")]

        message = digest({"window": WINDOW, "tickets": [], "runs": runs, "prs": []})

        assert "*0 tickets kept off the eng queue*" in message
