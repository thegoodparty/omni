...
LOCAL_IMAGE="serve-analyze"
TAG="${1:-latest}"

case "$TAG" in
  main|master|prod|qa|dev|release)
    ECR_TAG="serve-analyze-${TAG}"
...
echo -e "${BLUE}Building V1 Pipeline Docker Image${NC}"
...
if [ "$PUSH_TO_ECR" = "true" ]; then
  ...
  docker buildx build \
    --platform ${PLATFORM} \
    -f serve/v1_pipeline/Dockerfile \
    -t ${ECR_REPO_URL}:${ECR_TAG} \
    -t ${ECR_REPO_URL}:v1-pipeline-latest \
    --push \
    .
