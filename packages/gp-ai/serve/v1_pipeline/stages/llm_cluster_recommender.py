...
class Top3ClustersResponse(BaseModel):
    top_clusters: list[ClusterRecommendation] = Field(description="Top 3 most substantive, actionable clusters")
    overall_assessment: str = Field(description="1-2 sentences on the overall themes across all responses")


def _aggregate_by_theme(unified_records) -> dict[str, dict[str, Any]]:
    """Group records by theme and calculate metrics"""
    theme_data = defaultdict(
        lambda: {
            "messages": [],
            "unique_phones": set(),
            "sentiments": [],
    ...
    cluster_summary = {}
    total_respondents = len({r.phone_number for r in unified_records})
    ...
    for theme, data in theme_data.items():
        ...
        cluster_summary[theme] = {
            ...
            "avg_confidence": round(sum(data["confidence_scores"]) / len(data["confidence_scores"]), 3)
...
def _format_clusters_for_llm(cluster_summary: dict[str, dict[str, Any]]) -> str:
    ...
    for idx, (theme, data) in enumerate(
        sorted(cluster_summary.items(), key=lambda x: x[1]["unique_respondents"], reverse=True), 1
    ):
...
async def recommend_top_clusters_via_llm(unified_records, config: dict[str, Any]) -> tuple:
    ...
    filtered_summary = {
        theme: data for theme, data in cluster_summary.items() if data["unique_respondents"] >= min_respondents
    }

    logger.info(
        f"Aggregated {len(cluster_summary)} themes, {len(filtered_summary)} meet min_respondents threshold ({min_respondents})"
    )
    ...
    llm_client = Gemini3Client(default_model=model, default_temperature=temperature, thinking_level=thinking_level)
