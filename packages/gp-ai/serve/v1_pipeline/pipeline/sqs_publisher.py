...
class SQSEventPublisher:
    ...
    async def publish_poll_completion(
        ...
        for poll_id in poll_ids:
            ...
            if records:
                ...
                unique_respondents = len({r.phone_number for r in records if not r.is_opt_out})
    ...
    def _rank_clusters(self, cluster_stats: dict[int, dict]) -> list[dict]:
        filtered = [c for c in cluster_stats.values() if c["responseCount"] >= self.min_respondents]
