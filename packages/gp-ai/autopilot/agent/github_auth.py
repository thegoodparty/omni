"""Mints a GitHub App installation token for the agent's clone/push access.

Copied from engineer_agent/agent/github_auth.py (not imported): the two agents
deploy independently, and this file is small enough that a shared import would
couple their deploys for no benefit. Keep the two in sync by hand if the
Delegate App's auth mechanics ever change.
"""
