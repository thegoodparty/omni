...
export const controllerAlerts = (controller: ControllerName): Alert[] => {
  ...
  return routes.map((route) => {
    ...
    return {
      ...
      message: [
        serverErrorsOnly
          ? `\`${route.endpoint}\` returned server errors in the last ${LOOKBACK_PROSE} (status ≥ 500). 4xx responses are deliberately excluded on this controller — see SERVER_ERRORS_ONLY in alerts.ts.`
          : `\`${route.endpoint}\` returned unexpected error responses in the last ${LOOKBACK_PROSE} (status ≥ 400, excluding 401/403/404/409/498).`,
