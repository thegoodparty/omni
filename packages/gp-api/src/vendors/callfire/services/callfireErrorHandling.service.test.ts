...
describe('CallfireErrorHandlingService', () => {
  let service: CallfireErrorHandlingService

  beforeEach(() => {
    service = new CallfireErrorHandlingService()
  })
  ...
  it('maps an axios error to a 502 without echoing the upstream body', () => {
    const err = axiosError(400, { message: 'account 12345 secret' })
    ...
    expect(message).toBe('CallFire API error')
    expect(message).not.toContain('secret')
  })

  it('uses the caller customMessage when provided', () => {
    const err = axiosError(500, { message: 'throttled' })

    let thrown: unknown
    try {
      service.handleApiError({
        error: err,
        customMessage: 'CallFire number rental failed',
  ...
  describe('permanent vs transient classification', () => {
    ...
    it.each([400, 402, 403, 404])('classifies %i as permanent', (status) => {
      expect(classify(status)).toBeInstanceOf(VendorPermanentError)
    })
    ...
    it.each([401, 408, 429, 500, 502, 503])(
      ...
      (status) => {
        ...
        expect(thrown).toBeInstanceOf(BadGatewayException)
        expect(thrown).not.toBeInstanceOf(VendorPermanentError)
      },
    )
  })
})
