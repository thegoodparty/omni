...
import { CallfireBaseConfig } from '../config/callfireBaseConfig'
...
// Thin wrapper over the CallFire REST API: attaches HTTP Basic auth
// (base64(login:password)) + base URL + timeout, and retries the 429 throttle
// (and transient 5xx on idempotent GETs) with a short backoff. Callers do the
// Zod parsing and error mapping.
@Injectable()
export class CallfireHttpService extends CallfireBaseConfig {
  constructor(
  ...
  // retried for idempotent GETs — retrying a POST (rent, import, upload) could
  // double a billable side effect.
  ...
  async put<T>(
    ...
    // 5xx — a PUT that transitions state may have already executed, and
    // re-sending could re-trigger the side effect.
    const res = await this.withRetry<T>(
      () =>
        firstValueFrom(
