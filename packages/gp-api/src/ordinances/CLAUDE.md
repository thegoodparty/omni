...
| `ordinances.constants.ts` | Loop constants (`MAX_QUALITY_LOOP_REVISIONS`, pinned `QUALITY_LOOP_MODELS`, timeouts), flag + env-var names |
...
- `start()` guard order: flag (`serve-ordinance-quality-loop`, checked once) →
  `ORDINANCE_QUALITY_LOOP_ENABLED` env → `status === draft` → no redline
  (`{-`/`{+`) → non-empty draft → not already passing for the current hash →
  no live manual run → atomic conditional-`updateMany` claim (auto trigger
  first fence-flips the old run to `superseded_by_edit`). Enqueue failure →
  fenced flip to `failed`.
