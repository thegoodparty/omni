...
import { CommunityIssueFeedService } from 'src/communityIssueFeed/services/communityIssueFeed.service'
...
export class QueueConsumerService {
  constructor(
    ...
    private readonly communityIssueFeed: CommunityIssueFeedService,
  ...
  private async handleAgentExperimentResult(data: AgentExperimentResultData) {
    ...
    if (updatedRun.status === ExperimentRunStatus.COMPLETED) {
      ...
      await this.communityIssueFeed
        .onExperimentRunCompleted(updatedRun)
        .catch((err: unknown) =>
          this.logger.error(
            { err, runId: updatedRun.runId },
            'communityIssueFeed.onExperimentRunCompleted failed after run update',
          ),
        )
    }
