...
export class AiCampaignManagerIntegrationService extends createPrismaBase(
  ...
  async generateCampaignTasks(
    ...
    try {
      this.logger.log(`Starting AI task generation for campaign ${campaign.id}`)

      const request = this.buildCampaignPlanRequest(campaign)
      this.logger.log(
        `Built campaign plan request:`,
      ...
      if (existingTasks) {
        this.logger.log(
          `Using existing plan with ${existingTasks.length} tasks for campaign ${campaign.id}`,
        )
        return existingTasks
      }

      this.logger.log(
        `No existing plan found, generating new one for campaign ${campaign.id}`,
      )
      ...
      this.logger.log(
        `Downloaded campaign plan JSON with ${campaignPlanJson.tasks?.all_tasks?.length || 0} tasks`,
      )

      await this.saveCampaignPlan(campaignPlanJson, campaign, request)

      const parsedTasks = this.parseCampaignPlanToTasks(
        campaignPlanJson,
        campaign,
      )
      this.logger.log(
        `Parsed ${parsedTasks.length} tasks from campaign plan for campaign ${campaign.id}`,
      )

      return parsedTasks
    } catch (error) {
      this.logger.error(
        `Failed to generate campaign tasks for campaign ${campaign.id}:`,
        error,
      )
      throw error
    }
  ...
  private parseCampaignPlanToTasks(
    ...
    this.logger.log(`Parsing campaign plan JSON for campaign ${campaign.id}`)
    this.logger.log(`Campaign plan structure:`, {
      has_tasks: !!campaignPlanJson.tasks,
      all_tasks_length: campaignPlanJson.tasks?.all_tasks?.length || 0,
      timeline_length: campaignPlanJson.tasks?.timeline?.length || 0,
      voter_contact_length: campaignPlanJson.tasks?.voter_contact?.length || 0,
    })

    if (campaignPlanJson.tasks?.all_tasks?.length > 0) {
      this.logger.log(
        `Found ${campaignPlanJson.tasks.all_tasks.length} tasks in campaign plan JSON`,
      )

      campaignPlanJson.tasks.all_tasks.forEach((task, index) => {
        const convertedTask = this.convertJsonTaskToCampaignTask(
          task,
          campaign,
          index + 1,
        )
        this.logger.log(`Converted task ${index + 1}:`, {
          id: convertedTask.id,
          title: convertedTask.title,
          week: convertedTask.week,
          flowType: convertedTask.flowType,
        })
        tasks.push(convertedTask)
      })
    } else {
      this.logger.warn(
        `No tasks found in campaign plan JSON for campaign ${campaign.id}, creating default tasks`,
      )
      const defaultAiTasks = this.createDefaultTasks(campaign.id)
      this.logger.log(`Created ${defaultAiTasks.length} default AI tasks`)
      tasks.push(...defaultAiTasks)
    }

    this.logger.log(
      `Returning ${tasks.length} total parsed tasks for campaign ${campaign.id}`,
    )
    return tasks
  }
  ...
  private convertJsonTaskToCampaignTask(
    ...
    return {
      ...
      flowType:
        (jsonTask.category as CampaignTaskType) || CampaignTaskType.education,
      week: weekNumber,
      link: jsonTask.link || undefined,
      proRequired: jsonTask.pro_required || false,
