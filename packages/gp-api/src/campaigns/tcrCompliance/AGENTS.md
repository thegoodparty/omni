...
Schedules differ by sweep, and the difference matters. `@Interval` is an
in-process timer: it fires in **every** ECS replica (prod runs 2), at whatever
boot-offset minute the task started, and it resets on every deploy. That is
tolerable for the cheap DB-only sweeps, but the two Peerly sweeps call the
vendor once per in-flight record, so per-replica firing doubled our request rate
into CampaignVerify. Both are now `@Cron` on a fixed wall clock (off-the-hour
minutes so they don't pile onto other on-the-hour jobs), leased through
`CronLockService.tryClaimHourlyRun` — every replica fires, exactly one wins the
`(jobName, runDate)` row for that UTC hour, and the loser returns immediately.
The winner seals its lease in a `finally` so a mid-pass throw can't leave the
claim dangling.

| Sweep                          | Schedule                               |
| ------------------------------ | -------------------------------------- |
| `sweepStrandedAgenticKickoffs` | `@Interval` 10 min (per replica)       |
| `sweepUnsubmittedUsecases`     | `@Cron('17 * * * *')` ET, hourly lease |
| `sweepPinDeliveryDetection`    | `@Cron('42 * * * *')` ET, hourly lease |
| `bootstrapTcrComplianceCheck`  | `@Interval` 12 h (per replica)         |
