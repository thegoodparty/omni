...
- **Single-key `details` writes go through `patchCampaignDetails`, which merges server-side.** The invariant that every such write routes through that one method was established by commit `230e1dae2`; what it now guarantees is atomicity. `patchCampaignDetails` issues one `UPDATE campaign SET details = details || $1::jsonb`, so concurrent patches of different keys cannot clobber each other. Do **not** reintroduce a read-modify-write: reading the blob, spreading a patch over it in application memory and writing it back is a lost update no isolation level fixes, because the read is a separate round trip. `Serializable` around the write alone bought nothing and was removed. `||` is a top-level merge (a nested object in the patch replaces rather than merges) and an explicit `null` writes JSON null, which `persistCampaignProCancellation` depends on. A missing campaign is a `NotFoundException`. A `details` that is not a JSON object is coerced to `{}` rather than refused — the merge also runs inside `setIsPro`'s transaction, where a throw would roll back the Pro flip and strand a payment on every Stripe redelivery.
- **`updateJsonFields` is the multi-field path and still reads inside its transaction**, because `deepMerge` plus its stale-election-result key deletions have no single-statement jsonb equivalent. There `Serializable` is load-bearing and the loser aborts with `P2034` instead of losing an update, so it is retried (`retryIf` + `isSerializationError`) — but only when it owns the transaction, never on a caller-supplied `outerTx`. `setIsPro` is retried the same way: its transition read and flip must serialize, and it merges the `isProUpdatedAt` stamp inside that same transaction so the flip and the date commit together.
