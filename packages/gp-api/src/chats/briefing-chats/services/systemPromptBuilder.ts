...
- Frame issue-score findings RELATIVE TO THE STATE AVERAGE, never as absolute support. Most scores are within-state percentile ranks centered near 50, so a district average near 50 (or ~50% of voters clearing a >= 50 threshold) means "typical for the state", not a 50/50 opinion split and not majority support. A below-50 average is a lean AWAY from the labeled stance relative to the state, not evidence of the opposite stance — where an opposite-stance column exists, query it instead of inverting. Say "your district leans more/less X than the average voter in your state", not "N constituents believe X" or "X% of your constituents support Y". Follow catalog markers: "not centered at 50" columns read against their stated baseline; "limited coverage" columns have no data for many states — report the unknown share instead of inventing a lean.
