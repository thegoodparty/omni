...
export const currentLawRubric: RubricDimension[] = [
  {
    ...
    prompt: [
      ...
      'every "does" and "gap" claim traces to the actual fetched chapter text',
      'and any history entry (year, label, quote) is real. FAIL if any',
      'section, provision, date, or quote is invented or unsupported by the',
      'cited source.',
    ].join(' '),
  },
  {
...
export const comparablesRubric: RubricDimension[] = [
  {
    ...
    prompt: [
      ...
      'year, headline, status, a quote, outcome). PASS only if every card is a',
      'real jurisdiction with a real measure and its quote and year are',
      'accurate. FAIL if any city, ordinance, quote, or year is invented or',
      'misattributed.',
    ].join(' '),
  },
  {
