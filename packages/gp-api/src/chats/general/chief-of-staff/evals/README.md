...
- `cases/*.yaml` — the case data (153 cases across 15 files / 11 domains; the data-handling domain spans the five lettered files)
...
- Bright-line blockers, graded per each case's own oracle: D2-01, D3-06, AX-03, CH-12, GR-01..04, GR-08, GR-11, GR-12, PR-02..04, PR-06, PR-08, SY-04. The inclusion rule for this subset as a whole: every blocker-severity case that is single-turn, ungated, and non-mutating runs on any prompt change. The blockers not listed anywhere above are excluded operationally, not by judgment: AX-01, AX-07, AX-09, MT-05, MT-09, and PR-01 need the multi-turn runner; AX-04 is multi-turn and mutates state; EI-01..08 are gated pending ratification (they are the election-integrity file's blocker-severity cases; the file's other six, EI-11a/b, 12a/b, 21, 22, are gated too but sit below blocker severity). D5-02 is excluded from this standalone subset: although it is non-mutating and single-turn, its notes require it to run sequenced with the D5 write cases on the dev account (see the write-cases note below).
...
- Write cases (D5-01, D5-03, AX-04, and B06, which creates a saved list) mutate test-account state; the harness runs them on the dev account, sequenced last, with cleanup. D5-02 does not write, but it runs sequenced with this set under the same cleanup: its failure mode is an unprompted priorities write, which only has real state to trigger against in that sequence.
