...
type MockReply = Parameters<VoterDownloadService['streamPeopleCsv']>[1]
...
describe('VoterDownloadService', () => {
  let service: VoterDownloadService
  ...
  describe('database URL swap', () => {
    it('ends the previous pool and builds a new one with the new URL', () => {
      ...
      const onChangeCallback =
        peopleDbUrlProviderMock.onChange.mock.calls[0]?.[0]
      if (!onChangeCallback) {
