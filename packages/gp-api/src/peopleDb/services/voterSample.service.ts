...
import { BaseDbPerson, buildVoterSelectSql } from '../voter.select'
import { DistrictService } from './district.service'
import { z } from 'zod'
import { StatsService } from './stats.service'
import { hash32 } from '../util/hash.util'
import { resolveDistrict } from '../utils/resolveDistrict.util'
import { stateEquals } from '../utils/buildVoterWhereSql.util'
...
@Injectable()
export class VoterSampleService extends createPeopleDbBase(
  PEOPLE_MODELS.Voter,
) {
  constructor(
  ...
  private async computeHashDivisorAndPrelimit(
    ...
    const desiredRows = targetSampleSize * VoterSampleService.OVERSAMPLE_FACTOR
    const dontBucketCutoff =
      desiredRows * VoterSampleService.MIN_POPULATION_MULTIPLIER

    const hashDivisor = Math.floor(
      (remainingConstituentCount * VoterSampleService.BUCKET_COUNT) /
        desiredRows,
    )

    const prelimit = Math.min(
      VoterSampleService.MAX_PRELIMIT,
      desiredRows * VoterSampleService.PRELIMIT_MULTIPLIER,
    )

    if (remainingConstituentCount <= dontBucketCutoff)
      return { hashDivisor: VoterSampleService.MIN_HASH_DIVISOR, prelimit }

    return {
      hashDivisor: Math.min(
        Math.max(hashDivisor, VoterSampleService.MIN_HASH_DIVISOR),
        VoterSampleService.MAX_HASH_DIVISOR,
      ),
      prelimit,
    }
  }
  ...
  private makeHashBuckets(hashDivisor: number, seed32: number): number[] {
    ...
    for (let i = 0; i < VoterSampleService.BUCKET_COUNT; ++i) {
