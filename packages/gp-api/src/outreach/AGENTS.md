...
| Route                                                                                                                   | Where                                 | What                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /outreach` (multipart) + `GET /outreach`                                      | `outreach.controller.ts`              | Legacy create/list + edit-before-send. Create accepts `draft: true` (p2p only) → row stored `pending_payment`, hidden from the list. Image required for text/p2p. p2p create (draft included) enforces the CAS compliance list deterministically via `requireCompliantScript` (contracts `checkSmsStandards`: opt-out, {first_name}, candidate name, paid-for-by + committee token, length) — a non-compliant message 400s before payment. There is NO candidate edit route any more (product decision 2026-09-02: CAS fixes messages via the admin console; candidates can only cancel); cancel is additionally blocked once `outreach.date` has passed                                                                                                                                                                                                                                                 |
...
| `GET /outreach/admin/sms/queue` + `GET /outreach/admin/sms/:id` + `POST /outreach/admin/sms/:id/approve` / `:id/deny` + `PATCH /outreach/admin/sms/:id` | `outreachSmsAdmin.controller.ts` | The CAS SMS console (gp-admin via `AdminOrM2MGuard`). The queue is every p2p row in the cancel window (spine `pending` + vendor job) with the standards verdict (`checkSmsStandards`, contracts), scrubbed counts, and live job readiness (one `getJobsByIdentityId` read per identity; a vendor failure renders `job: null`, never a 502). Approve is the one human gate: a claim CAS (two admins can't both book), `request_canvassers` at Peerly, then `canvassRequestedAt` — a vendor failure reverts the claim. Deny is the rare internal parking state: it stamps reason and never touches the vendor job, money, or the candidate (no email — product decision 2026-09-02, CAS fixes messages themselves). The usual fix path is the staff edit (`PATCH :id`, script only): stored-image bytes re-read from S3 (Peerly templates are a destructive overwrite), `updatePeerlyP2pJob`, then a CAS DB write on `pending` that swaps the script, KEEPS any booking/approval (the editor is the approver — product decision 2026-09-02), clears only a denial, and stamps `adminEditedAt/By`. Approve requests a CUSTOM 9am-9pm window, timezone LOCAL (relative to each contact). Decisions and edits Slack `cas-clickup-tasks` best-effort; approve fires `Voter Outreach - Campaign Approved`. Detail merges the live job + `detailedstats` counters (either read failing degrades to null). Shapes in contracts `SmsAdminConsole.schema.ts`, consumed via the SDK's `smsOutreachAdmin` resource |
| `POST /outreach/serve/phone-banking/draft`                                                                              | `outreachServePhoneBanking.controller.ts` | Serve counterpart to the phone banking draft above (ENG-10970): stateless, org-scoped via `@UseElectedOffice()`, never a campaign. Deliberately NOT Pro-gated: on the Serve surface the `ElectedOffice` row IS the entitlement — elected-office orgs are license-equivalent to Pro (`hasElectedOfficeAccess`) and phone banking is free, so there is no subscription tier to check (same model as the serve social routes below). Shares `OutreachPhoneBankingGenerationService` with Win, parametrized by a `PhoneBankingVoiceConfig` (elected-official purposes + You:/Voter: dialogue prompts); no date/campaign context on serve |
...
| `outreachSmsAdmin.service.ts`                        | The CAS console behind the admin routes above: queue/detail assembly (committee + candidate names from `tcr_compliance` feed the standards identity check), the approve claim/revert machinery, internal deny, and the staff script edit (S3 image re-read + canvass clear + vendor update + stamp wipe). Approval state lives on the spine (`approvedAt/By`, `deniedAt/By/Reason`, `canvassRequestedAt`, `adminEditedAt/By` — all nullable; candidates keep seeing "Scheduled") |
...
| `outreachRobocallStranded.service.ts`               | Stranded-`authorized` recovery (`OutreachRobocallStrandedService`, `createPrismaBase(MODELS.OutreachRobocall)`): a `@Cron` (`6,21,36,51 * * * *`, `EASTERN_TIMEZONE`) that finds drafts stuck in `authorized` with `callhubCampaignPkStr IS NULL` (never staged) whose `outreach.date <= now` (send passed) and fails each via `OutreachRobocallHoldService.failSend(outreachId, 'expired_unstaged')` — the one gap no other sweep catches (staging only stages future sends, send only dials staged drafts), which would otherwise strand the draft in `authorized` forever with its Stripe hold reserved and the spine still "Scheduled". `failSend` does the rest (voids/releases the hold, `send_failed` + spine `failed`, `SendFailed` email, CRITICAL alert). Deliberately does NOT match staged drafts (a staged past-due draft still dials when `ROBOCALL_SEND_ENABLED` is on — the send sweep's concern), `staging` (the staging stale-reclaim owns it), or `pending_payment`/`hold_failed` (the deferred-cancel and hold-failure sweeps own those). Per-record `try`/`catch`. **prod-only** (`OTEL_SERVICE_ENVIRONMENT`) but deliberately **NOT kill-switch-gated**: it only fails a definitively-undeliverable never-staged draft and only releases money, and a strand can occur regardless of the send/capture switches (staging is not switch-gated), so gating it would leave the reserved money stranded during the supervised rollout — same reasoning as the deferred-cancel and hold-reconcile sweeps; no `CronLockService` (failSend's single-owner CAS is idempotent across replicas). A SECOND `@Cron` on the same cadence, `sweepStrandedPaid`, is the estimate-billing (CONTINGENCY) analogue: it finds `paid` drafts (charge committed, `chargeIntentId IS NOT NULL`) with `callhubCampaignPkStr IS NULL` past their send and calls `OutreachRobocallChargeService.failStrandedEstimate` — `send_failed` + a `CRITICAL robocall` MANUAL-refund alert (this branch never auto-refunds) + a `SendFailed` email. **prod-only** (`OTEL_SERVICE_ENVIRONMENT`) but deliberately **NOT flag-gated**: the flag is designed to be flipped back OFF after real charges land (the supported rollback), and captured money must still be surfaced, so a flag-gated sweep would go inert and strand every already-`paid` run unalerted; removing the gate never changes hold-model behavior because the sweep only ever matches `paid` rows (which exist only where the flag charged an estimate). A THIRD `@Cron` on the same cadence, `sweepOrphanedEstimateClaims`, is the estimate-billing ORPHAN-CLAIM recovery: it finds `paid` drafts with `chargeIntentId IS NULL` whose `updatedAt` is older than `ROBOCALL_ESTIMATE_CLAIM_STALE_MINUTES` (a `chargeEstimate` that won the `pending_payment → paid` claim but crashed before its commit/decline/revert — the row stuck at `paid` with a charge Stripe may have CAPTURED never recorded, caught by no other sweep) and calls `OutreachRobocallChargeService.resumeStrandedEstimateCharge`, which RESUMES the charge under the SAME stable idempotency key + SAME frozen amount (a captured charge replays the SAME PaymentIntent — never a second charge; an un-landed one charges exactly once), commits `chargeIntentId` so the run becomes deliverable, routes a genuine decline to the same `charge_failed` + email terminal, and LEAVES a transient/card-revalidation failure `paid` for the next sweep (never reverting a possibly-captured orphan). It selects orphans regardless of send date (a future orphan must complete BEFORE its send). Same **prod-only, NOT-flag-gated** posture as `sweepStrandedPaid` (rollback safety): resuming completes the charge and lets the run dial via the normal send path, which is gated on `ROBOCALL_SEND_ENABLED`, never the billing flag — a `paid` run is an already-purchased run that dials exactly as before rollback; if it ever strands un-staged, `sweepStrandedPaid` surfaces it, so no captured-money path ends without an alert. All three sweeps share the cadence because their candidate sets are disjoint (`authorized`; `paid` + committed; `paid` + no charge). See "Robocall payment" below |
...
| `outreachServeComposeContext.service.ts`            | Serve counterpart to `outreachComposeContext.service.ts` (ENG-10982): builds prompt blocks from the elected official's own Public Profile materials (`PersonProfile.bioOverride`/`whyRunning`, `accomplishments`/`recentExperience` JSONB overlays, published `PersonProfileIssue`s — `visible: true`, `sortOrder` asc — joined to `Priority`). Reads by `ElectedOffice.userId` only, never a campaign table (the Win/Serve isolation boundary). Labels are agreed with product/politics and load-bearing — treat like an enforced contract, not a refactor target. Same trim discipline as the Win service (2000-char text cap, 300-char item cap, 5/5/10 list caps) but a separate service against different models — the Win service is untouched by this. Every block optional; a profile-less official gets today's name/office/place baseline unchanged |
...
estimate is FROZEN onto the row (`authorizedAmountInCents`, the hold model's own
frozen-estimate column, unused by this path otherwise) as part of the claim CAS
and REUSED on every later path — the retry, `capturedAmountInCents` at commit, the
idempotent replay amount — so a retry after a transient/lost-response failure can
never send a DIVERGENT amount under the shared idempotency key (which Stripe would
reject as keys-must-match, an error that would loop forever with money already
taken). The claim CAS ALSO persists `paymentMethodId` (not just at the success
commit) so an ORPHANED claim — a crash after the claim but before the
commit/decline/revert, leaving the row `paid` with `chargeIntentId` still NULL and
a charge Stripe may have CAPTURED unrecorded — carries the card the orphan-claim
recovery sweep (`sweepOrphanedEstimateClaims`, below) needs to RESUME the charge
under the stable key. Safe against the detach webhook: `NOT_YET_DIALED_STATES`
excludes `paid`, so a `paid` row with a card set is never selected for a
detach-cancel. The STAGING and SEND sweeps accept BOTH `authorized` and `paid` (a `paid`
draft is gated on `chargeIntentId IS NOT NULL` — never-dial-unpaid — and is
released back to `paid`, not `authorized`, so the send slice re-checks the charge,
not a hold); the completion sweep requires `authorizationIntentId IS NOT NULL`, so
an estimate-billed `dialed` row never enters settling/capturing (which would
double-charge). A `paid` draft whose send passes while still UN-staged
(`callhubCampaignPkStr` NULL) is caught by no staging/send sweep, so
`OutreachRobocallStrandedService.sweepStrandedPaid` (prod-only, NOT flag-gated —
so a rollback can't strand it) is its safety net: it fails the row to `send_failed`, logs a `CRITICAL robocall` line so ops is
paged for a MANUAL refund (this branch never auto-refunds), and emails the
candidate `SendFailed` — the estimate analogue of the stranded-`authorized` sweep.
A `paid` draft with `chargeIntentId` STILL NULL (an ORPHANED claim whose charge
never committed) is caught by neither the staging/send sweeps nor sweepStrandedPaid
(all require `chargeIntentId`), so `sweepOrphanedEstimateClaims` (prod-only, NOT
flag-gated) is ITS net: past the stale window it calls `resumeStrandedEstimateCharge`, which
RESUMES the charge under the SAME stable key + frozen amount (a captured charge
replays the SAME PaymentIntent — exactly-once completion) and commits
`chargeIntentId` so the run finally dials; a decline routes to `charge_failed` +
email, a transient/card failure leaves it `paid` for the next sweep (never
reverting a possibly-captured orphan). Nothing in the hold path is deleted; flip
the flag OFF to restore it exactly. The
response is `RobocallPayResponseSchema` (the union of the hold and charge shapes).
The admin records flow for an estimate-billed run that never dialed is a later
slice.
...
`OutreachMaterializationService.materializeOutreach(campaign, outreach)`
...
dialed` CAS matching at most once per outreach (a repeat commit attempt finds
the row no longer `dialing` and is skipped) PLUS
`ContactInteractionRobocall`'s `@@unique([outreachId, personId])` +
`createManyIdempotent`'s `skipDuplicates` as a second layer, so even a direct
re-invocation of the materializer is a no-op, not a duplicate write. No
circular DI: `OutreachMaterializationService` is already a plain provider in
...
(no rethrow, no retry); transient → the existing revert-claim + rethrow so the
sweep retries next pass. `failSend` is the single surface path: a
...
- **Robocall materializes recipients and captures no delivery outcome, and
  ...
  0013).** Two robocall paths reach `OutreachMaterializationService` and both
  leave those columns null. The legacy type (created via the generic `POST
  /outreach`, fulfilled out of band, off the legacy Slack) has no send step at
  all — CallHub is never called for it, and Peerly's integration is its P2P
  texting product (the CDR report is SMS-shaped), so there is no vendor read
  to sweep. VO 2.0's robocall (`outreachRobocall*.service.ts`) DOES dial
  through CallHub and materializes too — triggered from
  `OutreachRobocallSendService.commitDialed` at the actual dial commit (see
  above) — but it still never reads CallHub's per-call CDRs back into these
  rows: `ENG-10677` deliberately deferred that read. The two columns ARE
  written, but only by the manual per-person log
  (`ContactInteractionsController`, `outreachId: null` by design), so the same
  null means "logged as not answered" on a manual row and "never observed" on
  a campaign row from either path. `OutreachInboundSweep` is text/p2p only and
  must stay that way until the CDR-read question is settled — **do not build
...
endpoints; `outreachUpdate.test.ts` covers edit-before-send (vendor call +
row update, reschedule only on a date change, non-pending/jobless rejected,
vendor failure leaves the row untouched); `outreachServeSocial.test.ts`
covers the Serve counterpart (a draft per serve purpose asserting the serve
goal/system prompt and no candidate/voter framing, the Win-purpose-slug
rejection, the ElectedOffice 404 gate on every route, custom/improve, LLM
failure → 502), Public Profile grounding (ENG-10982: all five labeled blocks
verbatim, hidden `PersonProfileIssue`s excluded, the profile-less baseline
pinned byte-identical, over-cap trimming, the generate path receiving the
same blocks as draft), and the Win/Serve isolation matrix (disjoint lists,
cross-route 404s on detail, a serve save leaving an existing Win row
byte-unchanged, and archive/restore on a serve row that cannot reach a Win
row); `outreachPhoneBanking.test.ts` covers the phone-banking draft
...
`outreachSmsAdmin.test.ts` covers the CAS console (queue derivation +
standards verdict + vendor-failure degradation, admin gate, approve
claim/revert/409, internal deny, the staff edit (denial clear +
booking kept + no-image 400), and detail stats degradation);
