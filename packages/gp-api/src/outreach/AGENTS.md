...
- **Never dial twice.** A single-owner dial claim CAS (`authorized` AND
  ...
  → commit `dialed` (idempotent); **`paused`/`pending`** → it did not → revert
  ...
  `SendFailed` email; **`unknown`/read-fail** → LEAVE the row in `dialing` (never
  relaunch without a not-dialing read, never mark dialed without a
  dialing/completed/aborted read) for the stale-dialing sweep, and alert — UNLESS
  the launch error was a `VendorPermanentError`, in which case even a failed read
  resolves to `failSend(outreachId, 'send')`: a definitive 4xx guarantees the
  campaign never dialed. The permanent path first persists a
...
(`dialing`/`completed`/`aborted` → `dialed`, `paused`/`pending` → `authorized`,
else left `dialing`). It READS the hold only — no capture/void, no completion
poll.
...
not-dialing (`paused`/`pending`) read for either permanent kind, plus — for a
definitive 4xx (`permanent`) only — an unresolved read too (a rejected launch
never dialed). A launch that reads dialing/completed/aborted still commits
...
stamps it, skips an already-aborted one, isolates a per-campaign vendor failure
(others abort, the failed one stays unaborted to retry), stamps once under a
concurrent double-run, and no-ops off prod — mocking the `ROBOCALL_VENDOR` port
(`abortBroadcast`). The recording itself is asserted
