...
| `POST /outreach/social/draft` / `social/generate` / `social` (save), `GET /outreach/:id`, `GET /outreach/:id/receipt`, `POST /outreach/:id/cancel`, `DELETE /outreach/:id`, `PATCH /outreach/:id/archive` | `outreachSocial.controller.ts`        | Social flow (VO 2.0 phase 1): stateless draft/improve, per-platform asset generation, atomic save, detail read; receipt (live Stripe read off the stored checkout session, dollars; 404 free-texts rows, 502 on Stripe failure), cancel-before-send for paid SMS rows, and delete for canceled rows (the service enforces canceled-only), all campaign-scoped; archive/restore for the history drawer, org-scoped via `@UseOrganization()`                                                                                                                                                                                                   |
