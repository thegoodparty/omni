...
| Route                                                                                                                   | Where                                 | What                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /outreach` (multipart) + `PATCH /outreach/:id` (multipart) + `GET /outreach`                                      | `outreach.controller.ts`              | Legacy create/list + edit-before-send. Create accepts `draft: true` (p2p only) → row stored `pending_payment`, hidden from the list. Image required for text/p2p. PATCH edits a scheduled p2p row inside the cancel window (status `pending` + vendor job): name, script, send date, optional replacement image — never the audience (frozen and priced at checkout; that path is cancel-and-recreate). Vendor first, then a CAS DB write on `pending`: an outstanding canvass request is `clearCanvassers`-ed FIRST (fail closed — an edited message must never ride a stale approval), then `updatePeerlyP2pJob` re-creates media (templates are a destructive overwrite) and, on a date change, mints a fresh schedule and repoints `schedule_id` + start/end dates (Peerly has no schedule-update endpoint); the DB write also nulls every approval/denial stamp so the edit re-queues for manual CAS reapproval                                                                                                                                                                                                                                                 |
...
claim/revert/409, internal deny, the staff edit (stamp wipe + fail-closed
clear + no-image 400), detail stats degradation, and the
edit-clears-approval + fail-closed-clear interplay on the candidate PATCH);
