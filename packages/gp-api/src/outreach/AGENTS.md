...
with a live orphan hold is voided + reverted to `pending_payment` with
`payAttempt++`; a stranded row with no live hold reverts without a void; the
revert clears any stale authorization fields; a search anomaly returning >1 hold
voids them all; a fresh (not-yet-stale) row is left `hold_pending`; a Stripe
search failure leaves the row `hold_pending` (no void, retries next sweep); a
concurrent double-sweep elects one recoverer; recovery runs even with
...
`StripeService.findLiveManualHoldsByOutreach` and `voidHold`;
