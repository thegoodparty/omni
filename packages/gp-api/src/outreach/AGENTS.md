...
`OutreachRobocallStrandedService.sweepStrandedPaid` (flag-gated) is its safety
net: it fails the row to `send_failed`, logs a `CRITICAL robocall` line so ops is
...
(all require `chargeIntentId`), so `sweepOrphanedEstimateClaims` (flag-gated) is
ITS net: past the stale window it calls `resumeStrandedEstimateCharge`, which
