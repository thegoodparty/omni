...
| Route                                                                                                                   | Where                                 | What                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /outreach` (multipart) + `PATCH /outreach/:id` (multipart) + `GET /outreach`                                      | `outreach.controller.ts`              | Legacy create/list + edit-before-send. Create accepts `draft: true` (p2p only) → row stored `pending_payment`, hidden from the list. Image required for text/p2p. PATCH edits a scheduled p2p row inside the cancel window (status `pending` + vendor job): name, script, send date, optional replacement image — never the audience (frozen and priced at checkout; that path is cancel-and-recreate). Vendor first, then a CAS DB write on `pending`: `updatePeerlyP2pJob` re-creates media (templates are a destructive overwrite) and, on a date change, mints a fresh schedule and repoints `schedule_id` + start/end dates (Peerly has no schedule-update endpoint)                                                                                                                                                                                                                                                 |
