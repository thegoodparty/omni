...
| `GET /outreach/admin/sms/queue` + `GET /outreach/admin/sms/:id` + `POST /outreach/admin/sms/:id/approve` / `:id/deny` | `outreachSmsAdmin.controller.ts` | The CAS SMS console (gp-admin via `AdminOrM2MGuard`). The queue is every p2p row in the cancel window (spine `pending` + vendor job) with the standards verdict (`checkSmsStandards`, contracts), scrubbed counts, and live job readiness (one `getJobsByIdentityId` read per identity; a vendor failure renders `job: null`, never a 502). Approve is the one human gate: a claim CAS (two admins can't both book), `request_canvassers` at Peerly, then `canvassRequestedAt` — a vendor failure reverts the claim. Deny stamps reason + emails the candidate (best-effort) and never touches the vendor job or money; the row re-queues when the candidate edits. Both decisions Slack `cas-clickup-tasks` best-effort; approve fires `Voter Outreach - Campaign Approved`. Detail merges the live job + `detailedstats` counters (either read failing degrades to null). Shapes in contracts `SmsAdminConsole.schema.ts`, consumed via the SDK's `smsOutreachAdmin` resource |
...
| `outreachSmsAdmin.service.ts`                        | The CAS console behind the admin routes above: queue/detail assembly (committee + candidate names from `tcr_compliance` feed the standards identity check), the approve claim/revert machinery, deny + candidate email. Approval state lives on the spine (`approvedAt/By`, `deniedAt/By/Reason`, `canvassRequestedAt` — all nullable; candidates keep seeing "Scheduled") |
...
claim/revert/409, deny + candidate email, detail stats degradation, and the
edit-clears-approval + fail-closed-clear interplay on PATCH);
