...
persisted card to `hold_failed` + one `HoldFailed` milestone (and does not
re-select it next sweep), marks a declined draft `hold_failed` without a retry
