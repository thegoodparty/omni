...
- **Never dial twice.** A single-owner dial claim CAS (`authorized` AND
  ...
  guarantees the campaign never STARTED, so leaving it `dialing` would only loop
  it through the stale sweep (which reconciles WITHOUT the permanent flag) into
  another permanent reject. A commit-miss (the draft moved underneath) means the campaign may be
...
phone was dialed, so nothing is owed), flips the SPINE `Outreach.status → failed`
...
- **INV-1.** The charge is `min(calcRobocallAmountInCents(completedCallCount),
  authorizedAmountInCents)` — a fresh charge can never exceed what the candidate
...
window-fit → `window_fit` and lost-commit → `lost_commit` voids),
`outreachRobocallCapture.test.ts` (zero-billable → `zero_billable`), and
