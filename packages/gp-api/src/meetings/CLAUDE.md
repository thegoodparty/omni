...
The daily cron (`dispatchDailyBriefings`, `0 7 * * *` UTC) sweeps every `ElectedOffice`, and dispatches a `meeting_briefing` run for any office whose next future `MeetingBriefing` row is missing — but only when the schedule projects a meeting within the 3-day imminence window (`IMMINENCE_WINDOW_DAYS`). Manual dispatches without the gate use a 60-day window instead. The cron also skips offices whose election-api position is explicitly not serve-ICP (`isServeIcp === false`, sourced from Databricks `int__icp_offices` via `resolveServeContext`); null/unknown fails open until the ICP backfill lands, and manual dispatches are never ICP-gated.
