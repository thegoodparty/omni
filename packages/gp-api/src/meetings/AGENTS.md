...
The daily cron (`dispatchDailyBriefings`, `0 7 * * *` UTC) sweeps every `ElectedOffice`, and dispatches a `meeting_briefing` run for any office whose next future `MeetingBriefing` row is missing — but only when the schedule projects a meeting within the 3-day imminence window (`IMMINENCE_WINDOW_DAYS`). Manual dispatches without the gate use a 60-day window instead. `dispatchManual` called with `useImminenceGate: true` means "match the cron exactly"; the ungated brief-now path skips the imminence and coverage checks.

**Activity gate.** After the imminence + coverage + in-flight-run gates pass, the shared `dispatchBriefingIfNeeded` also checks `metaData.lastVisited` on the office's user via `isInactiveUser` (`src/shared/util/userActivity.util.ts`, shared with `communityIssueDispatch.service.ts`): if it's absent or more than `INACTIVITY_THRESHOLD_DAYS` (30) old, the cron skips the dispatch and fires `Briefing Assistant - Dispatch Skipped` (feeds a HubSpot re-engagement email) instead of spending generation budget on an inactive user. The self-serve `dispatchBriefingIfDue` (backing `POST /meetings/dispatch-if-needed`) calls the exact same function with `skipActivityGate: true` — landing on the dashboard already proves activity — so it catches the dispatch up the moment the user returns.
...
| POST   | `/meetings/briefings/seed`             | Preview/dev-only. Writes a complete, schema-shaped briefing artifact to S3 plus a `COMPLETED` `ExperimentRun` and a `MeetingBriefing` row, so e2e tests get deterministic data without an agent run. Fails closed on qa/prod via the `OTEL_SERVICE_ENVIRONMENT` allow-list in `briefingSeed.service.ts`. Upserts on `(electedOfficeId, meetingDate)`, so re-seeding a date is idempotent. |
...
| GET    | `/meetings/briefings/dispatch/preview` | Read-only gate preview for a briefing dispatch: plays out the schedule RRULE and returns whether the imminence gate would dispatch, why not (coverage dedupe / no imminent meeting), and whether an ungated override would find a meeting in the 60-day window. Also reports the position's `isServeIcp` as diagnostic info; it does not gate dispatch. Shape: `BriefingDispatchPreview` in `@goodparty_org/contracts`. Consumed by gp-admin.                               |
| POST   | `/meetings/dispatch-if-needed`         | Self-serve landing catch-up. Resolves the office from the authenticated user (not an admin-supplied ID) and runs the same eligibility gate as the cron with the activity gate skipped — landing on the dashboard already proves activity. Returns `{ dispatched, inFlight, meetingDate }`; the webapp polls `GET /meetings` on `inFlight` until that date's `hasBriefing` flips.          |
...
There is no time-based stale-run sweeper: a `RUNNING` experiment_run whose Fargate task dies is reconciled to `FAILED` by the gp-ai ECS task-reaper (a `failed` callback keyed on `startedBy=run_id`), not by a gp-api cron.
...
- **Never commit a briefing pointer before its object exists.** `artifactBucket` / `artifactKey` are NOT NULL and every reader GETs them, so a row is a promise that an object is there. A row pointing at a missing or foreign bucket is unrecoverable from inside the product — it fails identically on every request until someone edits the database. Anything writing a `MeetingBriefing` must upload first and commit second, and seed writers must take the bucket from `util/seedBucket.ts` rather than naming one inline (a row carrying the literal bucket `"seed"` cost 768 failed requests over seven days in dev).
