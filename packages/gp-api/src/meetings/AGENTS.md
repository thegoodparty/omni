...
| GET    | `/meetings/briefings/dispatch/preview` | Read-only gate preview for a briefing dispatch: plays out the schedule RRULE and returns whether the imminence gate would dispatch, why not (ICP / coverage dedupe / no imminent meeting), and whether an ungated override would find a meeting in the 60-day window. Shape: `BriefingDispatchPreview` in `@goodparty_org/contracts`. Consumed by gp-admin.                               |
