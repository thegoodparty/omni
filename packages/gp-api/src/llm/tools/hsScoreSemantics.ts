...
export const HS_SCORE_SEMANTICS = `SCORE SEMANTICS (misreading these produces false claims):
  ...
  - On centered columns, threshold with >= 50 (moderate lean) or >= 70 (strong lean); mirror low-side segments with < 50 / <= 30 (lean away / strong lean away). Report leans relative to the state average ("your district leans more X than the average voter in your state"), never as "N constituents believe X" or "X% of constituents support Y". Never present a score average as a share of people. These bans cover EVERY hs_ column, centered or not: a threshold count on a shifted-baseline column is NOT a count of people with that trait (a >= 50 count on a ~60-baseline column runs ~60-65% by construction), so never report one as a headcount — describe non-centered columns only as leans against their stated baseline.
  - Nulls mean UNKNOWN, not "no". Every column has some: voters registered after the vendor's spring snapshot are unscored everywhere, and Texas (~72% scored) and Utah (~82%) trail every other state (90%+) on the nationwide column set (columns without a "limited coverage" mark; vendor-side) — nulls are unscored voters, not errors. Never fold them into a category — and never hide them either: unknowns are a reportable segment. When you break down or filter by any column, also count the unknowns (SUM(CASE WHEN col IS NULL THEN 1 ELSE 0 END) AS unknown_count) and surface them whenever they are a sizable share or behave differently on the metric at hand — unknown-status groups often differ systematically (e.g. lower turnout), which can itself be actionable for the user. Unknown counts follow the same small-cell suppression rule as any other count: report small ones as ranges, never exact numbers. For alignment shares, compute over SCORED rows only (AVG(CASE WHEN col >= 50 THEN 1.0 WHEN col IS NOT NULL THEN 0.0 END)) rather than silently mixing nulls into the denominator, and say how many were scored. That result is the share of SCORED PEOPLE whose modeled score clears the threshold — name it that way. It is NOT a share who hold the view, and the denominator is a set of scored records, never answers, responses, replies, or people who were asked: never describe it with survey language.`
