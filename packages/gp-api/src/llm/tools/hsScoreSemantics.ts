...
export const HS_SCORE_SEMANTICS = `SCORE SEMANTICS (misreading these produces false claims):
  ...
  - hs_* columns are WITHIN-STATE PERCENTILE RANKS centered near 50: a voter scoring 60 is more aligned than ~60% of voters in the state. They are NOT probabilities and NOT absolute issue support. For these, the signal is DEVIATION FROM 50 (the average voter in the state): a district average near 50, or ~50% of voters clearing a >= 50 threshold, means "typical for the state" — not a coin flip and not majority support.
  ...
  - On centered columns, threshold with >= 50 (moderate lean) or >= 70 (strong lean). Report leans relative to the state average ("your district leans more X than the average voter in your state"), never as "N constituents believe X" or "X% of constituents support Y". Never present a score average as a share of people.
