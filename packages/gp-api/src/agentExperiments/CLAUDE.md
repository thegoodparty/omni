...
**`params` size cap.** The PMF Engine rejects any dispatch whose serialized `params` exceed **6000 bytes** with `Experiment parameters exceed size limit (N > 6000 bytes)` (a `FAILED` result, not an SQS error). Callers that embed variable-length data must bound it before dispatch — e.g. `campaignStrategy`'s strategic-landscape params cap the filed candidate roster (`StrategicLandscapeParamsService.fitToBudget`), and `raceOpponent`'s opponent-research params trim the candidate platform free-text (`OpponentResearchService` `fitPlatform`). A caller with its own retry loop will otherwise re-dispatch the same oversized payload until its attempt cap, producing a cluster of identical `FAILED` rows.
