...
- **The SMS/Peerly CSV path honors only a subset of the persisted `VoterFileFilter` fields.** `customFiltersToQuery` in `voterFile/util/voterFile.util.ts` is a hardcoded switch over `CustomFilter` values; a filter field only reaches the Peerly query if it's both mapped in `peerly/utils/audienceMapping.util.ts` (`mapAudienceFields`) AND has a `case` here. Currently mapped against `public."VoterXX"` columns: audience/party/age/gender, `has_cell_phone`/`has_landline` (presence on `VoterTelephones_*Formatted`), and ethnicity (`EthnicGroups_EthnicGroup1Desc`, using the gp-api value vocabulary in `voters.service.ts` — `'European'`, `LIKE '%Asian%'`/`'%Hispanic%'`/`'%African%'`). Income/education/language/homeowner/veteran/children/marital are **deliberately excluded**: their `ConsumerData_*` columns aren't in `ALLOWED_COLUMNS` and their value vocabulary isn't confirmed against this table (people-api's `green.Voter` uses different column names and values — don't copy them). Map a new field only after confirming both the column and its value vocabulary on `VoterXX`.
