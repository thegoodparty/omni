...
- **A throttled availability check is not a taken domain.** Route53
  ...
  Any search that dropped candidates that way — or ran out of time budget —
  comes back with `partial: true`, meaning the list is a floor. Callers must
  not read a missing domain as taken when `partial` is set.
