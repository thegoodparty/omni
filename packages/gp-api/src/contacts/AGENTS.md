...
| `VOTER_DATA_UNAVAILABLE` on an org that clearly has a district | Two different failures share that error code: `resolveDistrictInfoFromOrg` found no district at all, **or** the district resolved but has no `DistrictStats` row (`fetchStatsByDistrictId`). Deliberate — they're the same user-facing state, and callers gate on the code, not the cause. Check people-db `DistrictStats` for the resolved `districtId` to tell them apart |
