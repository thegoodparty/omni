...
strips it, so without the mark the assistant reported a modeled estimate
...
| Nobody subsets by ethnicity       | A product rule, not a licensing one, and it binds BOTH products. The six `ethnicity*` booleans are gone from `voterFilterBaseSchema` and `ethnicity` from contracts' `PeopleFilters`, so no request can express it and the `.strict()` assistant tool 400s the key by name. The dimensions catalog no longer lists it, `convertVoterFileFilterToFilters` drops the six columns a pre-rule `VoterFileFilter` row still carries (such a list recounts without the clause rather than breaking mid-outreach), the legacy voter-file path stops emitting `ethnicity_*` (`voterFileFilter.service.ts`), the CSV download drops the Ethnicity column, `EthnicGroups_EthnicGroup1Desc` left `voter.select.ts` and the resident projection, the door-knocking pack dropped its `ethnicity` dim (`PACK_FORMAT_REVISION` 4), and both assistant prompts refuse the ask by name including proxies for it. Aggregate district composition is a different question and stays (`voters.service.ts`'s `getEthnicityCounts`). Fenced by `filterDimensions.catalog.test.ts` and `voterFileFilter.utils.test.ts` |
