...
| `POST /v1/contacts/overlap-count`                                                  | Saved-list overlap for the wizard's "N (P%) voters already exist in lists you've saved" strip (ENG-10840): the in-progress selection AND'd with the union of the org's saved lists (capped at the 25 most recent, truncation logged). Same in-progress payload as `count`, and open to a free campaign the same way                                                                                                                                                                     |
| `POST /v1/contacts/polygon-preview`                                                | Live count for a boundary drawn over an in-progress list: same unsaved-draft payload and gates as `count`, narrowed by a bbox prefilter plus an in-process ray-cast (people_db has no geometry column, so there is no ST_Contains to run). A boundary can have several parts (`GeoJsonShape` — one `Polygon`, or a `MultiPolygon`), and each part is scanned against its OWN bbox and unioned by person id, so the count is unique across parts and one box around scattered parts never drags most of the district through the cap. Returns `{ count, audienceEmpty }` — `audienceEmpty` separates "your filters match nobody" from "this shape holds none of your audience", which are the same zero on the wire and different problems. Over the query's cap it 400s rather than truncating                                                                                                 |
...
| `POST /v1/contacts/list-detail`                                                    | The same demographics + reachability for an UNSAVED filter — the body is the count endpoint's inline filter (`CountContactsDTO`), run through the same resolution. `outreachHistory` is always `[]`: nothing has been sent to a list that does not exist. Backs the voter data page's recommended-list detail sheet (`docs/features/recommended-lists.md`). Aggregates only, so open to a free campaign like `count` |
| `GET /v1/contacts/list-detail`                                                     | Saved-segment detail (`segment` param): demographics, reachable-by-channel (sms/robocall/phoneBanking/doorKnocking/polls), outreach history. Aggregates only, so open to a free campaign like `count` (the outreach audience step prices a saved list off it before the upgrade). Omitting `segment` returns the universe row's detail instead — the whole unfiltered district, `outreachHistory` always `[]` (ENG-10778). History excludes `doorKnocking` rows (the door-knock tool writes its own interaction rows) and orders null `date`s last with `createdAt` fallback fields (ENG-10776) |
| `GET /v1/contacts/download`                                                        | Streaming CSV export: a curated ~76-column subset with friendly headers (`DOWNLOAD_COLUMNS`, ENG-10766, widened in DATA-2281), not the raw L2 columns. Serve downloads drop party, turnout propensity, vote history, and ethnicity **columns** entirely via projection (`SERVE_EXCLUDED_DOWNLOAD_COLUMNS`, which is `EXCLUDABLE_VOTER_COLUMNS` verbatim, ENG-10830 + #1933) since a stream can't be post-processed                                                            |
...
| `POST /v1/voters/voter-file/filter`, `GET /filters`, `GET/PUT/DELETE /filter/:id`  | Saved-filter CRUD; PUT/DELETE 409 once locked. Create is not Pro-gated (outreach-pro-gating-v2, a free candidate can save a list before upgrading); PUT/DELETE still call `filterAccessCheck`                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `VoterFileFilterGeoMember`                     | The person-id set a drawn boundary resolved to, frozen at draw time. The GEOGRAPHIC half of a list, not its membership. `geoPoly` set with zero rows means the shape enclosed nobody and must resolve to empty, never to "no constraint". A boundary of several parts resolves to ONE set — a person standing where two parts overlap is one member, which is why the parts may overlap at all                                                                                                       |
...
`stripWinOnlyFieldsIfElectedOffice`) → respond. Typeahead is the same endpoint
...
description and by the Chief of Staff prompt's `CRM_TOOLS_RULES` —
...
| Party never reaches Serve         | `stripWinOnlyFieldsIfElectedOffice` (list + detail + typeahead), download drops party (plus turnout propensity + vote history, ENG-10830) via projection, party **filters** 400 (`assertNoPartyFilterForElectedOffice`), the door-knocking route payload nulls it per target (`doorKnockingServe.service.ts`, which also declares `isServe` so the two paper surfaces can read one field rather than re-derive the prefix), and the dimensions catalog hides party. A party value in any `eo-` response is a bug — there's a party-leak test suite, in `contacts.service.test.ts` and in `doorKnocking.routes.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                               |
| Ethnicity never reaches Serve     | Win-only, and a product rule rather than a licensing one. #1933 removed ethnicity subsetting from both products; the Win half was reverted and the rule now binds Serve alone. The catalog marks the dimension `modes: 'win'` so the Chief of Staff never learns it exists, `assertNoEthnicityFilterForElectedOffice` 400s the key at the same six choke points the party gate covers, `resolveSavedFilterSets` drops a pre-rule saved list carrying the columns rather than failing the whole union, `stripWinOnlyFieldsIfElectedOffice` removes `ethnicityGroup` from list/detail/typeahead, both CSV paths drop the column via projection (`EXCLUDABLE_VOTER_COLUMNS` and `SERVE_EXCLUDED_VOTER_FILE_COLUMNS`), and the door-knocking route payload nulls it per target. Aggregate district composition is a different question and stays (`voters.service.ts`'s `getEthnicityCounts`). The Chief of Staff prompt refuses the ask by name and refuses a proxy for it (language, surname, a neighborhood standing in); the Campaign Manager prompt carries no ethnicity rule, because Win may cut a list by the dimension |
...
| Party visible to a Serve org                                   | The choke points are `stripWinOnlyFieldsIfElectedOffice` + the download projection. Treat as a sev bug (license)                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Assistant refuses / weird tool behavior                        | Serve: tools register whenever `contacts` + the organization resolve, no flag. Win: the same, and only while the campaign row does not say `isPro: false`; a campaign without Pro gets no voter file tool at all, and the prompt says why from the product map. Locked-filter 409 surfaces as an explanation                                                                                                                                                                                                                                                                                                                                                                                                                                |
