...
/**
 * In-memory per-IP token bucket for the public `/v1/campaign-plan-shares/:campaignId/:fileName` endpoint.
 *
 ...
 * on `goodparty.org/api/v1/campaign-plan-shares/*`, and/or (b) replacing the UUID with
 ...
 */
@Injectable()
export class CampaignPlanSharesRateLimitGuard implements CanActivate {
  private readonly logger = new Logger(CampaignPlanSharesRateLimitGuard.name)
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (bucket.tokens < 1) {
      this.logger.warn(
        `Rate limit hit on /v1/campaign-plan-shares/:campaignId/:fileName from ${ip}; refusing further requests until refill.`,
      )
  ...
  private maybeSweep(now: number): void {
    if (
      ...
      CampaignPlanSharesRateLimitGuard.SWEEP_INTERVAL_MS
    ) {
      this.lastSweepMs = now
      this.sweepIdleBuckets(now)
    }
    if (this.buckets.size > CampaignPlanSharesRateLimitGuard.MAX_BUCKETS) {
  ...
  private forceEvictOldest(): void {
    const target = Math.floor(
      CampaignPlanSharesRateLimitGuard.MAX_BUCKETS *
        CampaignPlanSharesRateLimitGuard.FORCED_EVICTION_FRACTION,
    )
