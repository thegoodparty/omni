...
| `pro-without-subscription-drift.ts`                               | Read-only report of campaigns that are `is_pro` with no subscription behind them. `DATABASE_URL` only — no Stripe key needed       |
| `stripe-campaign-reconcile.ts`                                    | Walks Stripe's Pro subscriptions against `campaign.details.subscriptionId`; read-only drift report (refund candidates, duplicates incl. one person across two customer records, stale Pro) |
| `repair-orphaned-pro-subscriptions.ts`                            | Writes the three data repairs the reconcile report implies (re-link an orphan, repoint a wrong `customerId`, normalise `subscriptionCanceledAt` units). Dry-run default, `--apply` to write; driven by a reviewed input list, never its own classification; refuses de-Pro and cannot reach Stripe with anything but a GET |
