...
npx tsx ./_tmp_issues_analysis.ts && rm ./_tmp_issues_analysis.ts
```
...
3. Report, per type (top_community_issues / trending_issues) and per status
   ...
   per completed run.
4. Query `community_issue_feed` rows upserted by the cohort runs (join via
