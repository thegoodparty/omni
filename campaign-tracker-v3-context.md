...
- xlsx (latest): 58 tasks (22 live / 36 proposed); 51 Dynamic / 7 Static; 48
  content-pill / 3 generated-per-item / 7 static; tiers P1×24, P2×20, P3×9,
  P4×5.
...
- Code and maintenance: review the weekly task digest service and existing
  runbooks to understand the task generation logic; consider a cron job for
  updates to manage AI costs.
...
- The "cron job to manage AI costs" maps onto the digest's cron + SQS pattern:
  batch the expensive work on a schedule rather than on every page load.
...
- One batched call per candidate, cached; regenerate only on race change or
  explicit refresh — this is the meeting-notes "cron job to manage AI costs"
  (batch, never per page load).
